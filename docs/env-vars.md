...
| `J2K_CUDA_USE_OXIDE_TRANSCODE` | Routes supported reversible 5/3 transcode CUDA kernels through the cuda-oxide PTX when `j2k-cuda-runtime/cuda-oxide-transcode` is enabled and PTX was generated. Irreversible 9/7 transcode kernels remain on built-in CUDA C PTX. | Built-in CUDA C PTX | Experimental |
