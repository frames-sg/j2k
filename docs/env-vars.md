...
| `J2K_CUDA_USE_OXIDE_TRANSCODE` | Routes supported reversible 5/3 and single-item irreversible 9/7 transcode CUDA kernels through the cuda-oxide PTX when `j2k-cuda-runtime/cuda-oxide-transcode` is enabled and PTX was generated. Batched/fused 9/7 transcode kernels remain on built-in CUDA C PTX. | Built-in CUDA C PTX | Experimental |
