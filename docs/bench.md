...
- JPEG: selected 8-bit YCbCr sequential RGB8 WSI tile shapes can run Metal
  ...
  `BatchOp::RegionScaled` path. Initial CMYK/YCCK `Rgb8` and `Rgba8`
  ...
  4:4:4 `Rgb8` plus full/ROI/scaled/region-scaled `Rgba8` output, and
  16-bit APP14 RGB plus YCbCr 4:4:4 `Rgb16` plus full/ROI/scaled/region-scaled
  `Rgba16`
  ...
  fixture coverage is CPU-backed; malformed or broader four-component
  coverage, 12-bit CMYK/YCCK implementation, other 12-bit subsampled color
  support, stronger non-constant 12-bit oracle fixtures, and other SOF3
  16-bit color layouts stay structured
