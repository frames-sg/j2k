...
- JPEG: selected 8-bit YCbCr sequential RGB8 WSI tile shapes can run Metal
  ...
  grayscale `Gray16`/`Rgb16` full-image and non-scaled ROI output is CPU-backed,
  as is initial lossless SOF3 8-bit grayscale full-image output for predictors
  1-7. Expanded four-component coverage, broader 12-bit multi-component
  RGB/progressive/scaled support, and broader SOF3 precision/color/restart/ROI/
  scaled support stay structured unsupported until CPU parity lands; all new
  Metal routes stay disabled until benchmark-approved resident wins land.
