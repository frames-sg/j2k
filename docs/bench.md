...
- JPEG: selected 8-bit YCbCr sequential RGB8 WSI tile shapes can run Metal
  ...
  and YCbCr 4:4:4 `Rgb16` output is CPU-backed. Initial lossless SOF3 8-bit
  grayscale `Gray8` and 16-bit grayscale `Gray16` full-image/ROI/scaled/
  region-scaled output for predictors 1-7 is CPU-backed. Expanded
  four-component coverage, broader 12-bit subsampled color support, stronger
  non-constant 12-bit oracle fixtures, and broader SOF3 color/restart/row
  support stay structured
