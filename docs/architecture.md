...
color/restart/row support remain separate CPU parity work. Initial SOF3 8-bit
...
- Expanding JPEG compatibility through CPU parity first: initial CMYK/YCCK CPU
  ...
  support, and broader lossless SOF3 color/restart/row support remain active
  parity work. Initial SOF3 8-bit grayscale `Gray8` and 16-bit grayscale
  `Gray16` full-image/ROI/scaled/region-scaled decode for predictors 1-7 has
  landed. Metal acceleration for those classes is gated on
