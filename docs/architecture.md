...
Expanded four-component coverage, broader 12-bit RGB/progressive/scaled support,
...
- Expanding JPEG compatibility through CPU parity first: initial CMYK/YCCK CPU
  ...
  landed. Initial 12-bit extended sequential grayscale `Gray16` full-image and
  non-scaled ROI decode has landed, while expanded four-component coverage,
  broader 12-bit extended/progressive/scaled support, and broader lossless SOF3
  precision/color/restart/ROI/scaled support remain active parity work. Initial
  SOF3 8-bit grayscale full-image decode for predictors 1-7 has landed. Metal
  acceleration for those classes is gated on parity and measured resident wins.
