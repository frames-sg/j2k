...
- initial lossless SOF3 8-bit grayscale full-image/ROI/scaled/region-scaled CPU
  ...
  to `Rgb8`, 8-bit YCbCr 4:4:4 decode to `Rgb8`, and 16-bit APP14 RGB decode
  to `Rgb16`, including restart-coded APP14 RGB and YCbCr streams; 8-bit
  grayscale/RGB row streaming and 16-bit grayscale `Gray16` row streaming have
  landed, while lossless 16-bit YCbCr and other 16-bit color layouts remain
  open
...
- Add predictor-specific tests, restart-marker tests, malformed-stream tests,
  ...
  APP14 RGB color/restart coverage has landed for 8-bit `Rgb8` and 16-bit
  `Rgb16` output, and 8-bit YCbCr 4:4:4 `Rgb8` coverage has landed. 8-bit
  grayscale/RGB row streaming and 16-bit grayscale `Gray16` row streaming have
  landed. Malformed streams, lossless 16-bit YCbCr, other 16-bit color layouts,
  and broader precision coverage remain open.
...
- SOF3 CPU decode matches oracle output for committed predictor fixtures.
  ...
  output, 8-bit YCbCr 4:4:4 fixtures across full-image/ROI/scaled/
