...
- initial 12-bit extended sequential and progressive grayscale full-image/ROI/
  ...
  initial 12-bit APP14 RGB 4:4:4 full-image/ROI/scaled/region-scaled CPU
  decode to `Rgb16`; broader 12-bit YCbCr/subsampled color support and
...
- Add 12-bit quantized coefficient and IDCT output handling without truncating
  ...
  `Gray16`/`Rgb16` and APP14 RGB 4:4:4 `Rgb16` paths have landed for SOF1
  12-bit streams without restart markers.
- Prefer `Gray16` and `Rgb16` output for native precision.
  ...
  grayscale path, and direct APP14 RGB 4:4:4 `Rgb16` is available for the
  initial color path; 12-bit YCbCr and subsampled color remain open.
...
- Add ROI, scaled, region-scaled, and batch coverage after full-image parity.
  ...
  grayscale and APP14 RGB 4:4:4 paths; session-batch coverage has landed for
  the initial RGB path.
- Update `JpegOutputBuffer` and capability reporting for 16-bit output formats.
  ...
  `Extended12` grayscale `Gray16`/`Rgb16` and APP14 RGB 4:4:4 `Rgb16`
  CPU-eligible.
...
- 12-bit extended sequential CPU decode matches the reference oracle for
  ...
  channel-distinct APP14 RGB 4:4:4 fixture covering full-image/ROI/scaled/
...
- Extend progressive coefficient assembly to 12-bit precision.
  ...
  `Gray16` or expanded `Rgb16` output and APP14 RGB 4:4:4 `Rgb16` output.
- Support full-image first, then ROI/scaled through the same output machinery
  ...
  the initial grayscale and APP14 RGB 4:4:4 paths through full progressive
...
- Keep capability reasons separate for unsupported progressive 12-bit shapes
  ...
  `Gray16`/`Rgb16` and APP14 RGB 4:4:4 `Rgb16` CPU-eligible while keeping
  Metal/CUDA rejected; broader 12-bit progressive YCbCr/subsampled color
...
- 12-bit progressive full and viewport-shaped CPU outputs match oracle data.
  ...
  channel-distinct APP14 RGB 4:4:4 fixture covering full-image/ROI/scaled/
  region-scaled `Gray16`/`Rgb16` output as applicable. Non-constant
  external-oracle fixtures and 12-bit YCbCr/subsampled color remain open.
