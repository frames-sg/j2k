...
- initial lossless SOF3 8-bit grayscale full-image/ROI/scaled/region-scaled CPU
  ...
  to `Rgb8` and 16-bit APP14 RGB decode to `Rgb16`, including restart-coded
  APP14 RGB streams; 8-bit grayscale/RGB row streaming and 16-bit grayscale
  `Gray16` row streaming have landed, while lossless YCbCr and other 16-bit
...
- Add predictor-specific tests, restart-marker tests, malformed-stream tests,
  ...
  and APP14 RGB color/restart coverage have landed for 8-bit `Rgb8` and
  16-bit `Rgb16` output. 8-bit grayscale/RGB row streaming and 16-bit grayscale
  `Gray16` row streaming have landed. Malformed streams, lossless YCbCr, other
  16-bit color layouts, and broader precision coverage remain open.
...
- SOF3 CPU decode matches oracle output for committed predictor fixtures.
  ...
  output, plus 8-bit grayscale/RGB row streaming and 16-bit grayscale `Gray16`
  row streaming.
