...
use j2k_test_support::{fnv1a64_hex, pnm_bytes, wrap_jp2_codestream};
...
fn write_decode_fixture_variant(
    ...
    let bytes = match container {
        ...
        DecodeContainer::Jp2 => wrap_jp2_codestream(
            codestream,
            source_image.width,
            source_image.height,
            u16::from(source_image.channels),
            8,
            colorspace_enum(source_image.channels),
        ),
    };
...
Decode fixtures include raw codestream and JP2-container variants. Rows in \
`fixtures.tsv` carry `source_fnv1a64`, so those variants remain tied to the \
...
const fn colorspace_enum(channels: u8) -> u32 {
    match channels {
        1 => 17,
        _ => 16,
    }
...
mod tests {
    ...
    fn materializes_source_images_into_decode_and_encode_manifests() {
        ...
        assert!(fixture_manifest.contains("\thtj2k\tjp2\n"));
