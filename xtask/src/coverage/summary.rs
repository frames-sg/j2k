...
fn summary_document(input: &CoverageSummaryInput<'_>) -> serde_json::Value {
    ...
    let document = json!({
        ...
        "threshold_mode": input.lane.line_threshold_mode(),
        "behavioral_release_gate": if input.lane == CoverageLane::Metal {
            Some("cargo xtask release-metal")
        } else {
            None
        },
...
mod tests {
    ...
    fn summary_records_partitioned_schema_and_exact_source_sha() {
        ...
        assert_eq!(document["threshold_mode"], "release-gate");
        assert_eq!(
