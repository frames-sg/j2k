...
pub fn parse_batch_sizes(value: &str, label: &str) -> Result<Vec<usize>, String> {
    let values = value
        .split(',')
        .map(str::trim)
...
pub struct BatchSizeConfig {
    pub case_batch_sizes: Vec<usize>,
    pub mixed_batch_sizes: Vec<usize>,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct BatchSizeEnv<'a> {
    pub case_batch_sizes: &'a str,
    pub mixed_batch_sizes: &'a str,
    pub legacy_batch_sizes: &'a str,
    pub legacy_batch_size: Option<&'a str>,
}

pub fn batch_size_config_from_env(
    env: BatchSizeEnv<'_>,
    default_case_batch_sizes: &[usize],
    default_mixed_batch_sizes: &[usize],
) -> Result<BatchSizeConfig, String> {
    let case_batch_sizes = std::env::var(env.case_batch_sizes).ok();
    let mixed_batch_sizes = std::env::var(env.mixed_batch_sizes).ok();
    let legacy = legacy_batch_sizes_from_env(env)?;
    batch_size_config_from_values(
        case_batch_sizes.as_deref(),
        mixed_batch_sizes.as_deref(),
        legacy,
        env.case_batch_sizes,
        env.mixed_batch_sizes,
...
pub fn batch_size_config_from_values(
    ...
    case_batch_sizes_label: &str,
    mixed_batch_sizes_label: &str,
    default_case_batch_sizes: &[usize],
    default_mixed_batch_sizes: &[usize],
) -> Result<BatchSizeConfig, String> {
    let case_batch_sizes = if let Some(value) = case_batch_sizes {
        parse_batch_sizes(value, case_batch_sizes_label)?
    ...
    let mixed_batch_sizes = if let Some(value) = mixed_batch_sizes {
        parse_batch_sizes(value, mixed_batch_sizes_label)?
    } else if let Some(values) = legacy {
        values
    } else {
...
pub fn legacy_batch_sizes_from_env(env: BatchSizeEnv<'_>) -> Result<Option<Vec<usize>>, String> {
    if let Ok(value) = std::env::var(env.legacy_batch_sizes) {
        return parse_batch_sizes(&value, env.legacy_batch_sizes).map(Some);
    }
    if let Some(name) = env.legacy_batch_size {
        if let Ok(value) = std::env::var(name) {
            return parse_batch_sizes(&value, name).map(Some);
        }
    }
    Ok(None)
}
...
pub fn manifest_required_value(
    fields: &[&str],
    index: usize,
    name: &str,
    row_number: usize,
...
pub fn external_source_label(
    path: &Path,
    control_character_message: &str,
) -> Result<String, String> {
...
pub fn host_hardware_label() -> String {
...
pub fn is_publishable_license_status(status: &str) -> bool {
    matches!(
...
pub fn default_batch_sizes_present(batch_sizes: &[usize], default_batch_sizes: &[usize]) -> bool {
    default_batch_sizes
        .iter()
        .all(|required| batch_sizes.contains(required))
}

pub fn join_usizes(values: &[usize]) -> String {
    values
        .iter()
        .map(usize::to_string)
        .collect::<Vec<_>>()
...
pub fn join_string_labels(values: &[String]) -> String {
    if values.is_empty() {
        "none".to_string()
    } else {
        values.join(",")
...
pub fn append_na_columns(row: &mut Vec<String>, count: usize) {
    row.extend((0..count).map(|_| "NA".to_string()));
}

pub fn skipped_external_mixed_prefix(
    tool_label: &str,
    batch_name: &str,
    method_label: &str,
) -> Vec<String> {
...
pub fn append_batch_input_columns(
    row: &mut Vec<String>,
    batch_size: usize,
    repeats: usize,
...
pub fn join_tsv_row(row: &[String]) -> String {
    row.join("\t")
}

pub fn mib_per_second(bytes: usize, elapsed_us: f64) -> f64 {
    if elapsed_us <= 0.0 {
        return 0.0;
    }
