...
#[expect(
    ...
    reason = "the staged group boundary keeps retained workers, image owners, flattened payloads, and dense output slots explicit"
)]
pub(super) fn run_staged_typed_group<T: Copy + Send>(
    ...
    for window_start in (0..image_jobs.len()).step_by(CPU_ENTROPY_IMAGES_PER_WINDOW) {
        let window_end = window_start
            ...
            .min(image_jobs.len());
        let window = StagedWindowContext {
            group,
            route,
            tile_options,
            image_range: Range {
        ...
        let tile_count = prepare_staged_window(
            workers,
            staged,
            flattened,
            &window,
        ...
        materialize_staged_window(workers, staged, &window, image_jobs, image_results, convert)?;
    }
    Ok(())
}

fn prepare_staged_window<T: Copy + Send>(
    workers: &mut [BatchWorker],
    staged: &mut CpuStagedWorkspace,
    flattened: &CpuGroupFastWorkspace,
    window: &StagedWindowContext<'_>,
    image_jobs: &mut [CpuTypedDecodeJob<'_, '_, T>],
    image_results: &mut [Option<Result<(), J2kError>>],
) -> Result<usize, BatchInfrastructureError> {
    let image_range = window.image_range.clone();
    let window_start = image_range.start;
        ...
        run_retained_chunks(
            ...
            |worker, jobs, results| {
                ...
                for (job, result) in jobs.iter_mut().zip(results) {
                    ...
                    *result = Some(prepare_staged_image(
                        job.image,
                        window.route,
                        worker,
                        &mut scratch,
    ...
    let execution = staged.execution();
    for (local_index, job) in image_jobs[image_range].iter().enumerate() {
        if image_results[job.slot].as_ref().is_some_and(Result::is_err) {
            continue;
        }
        match staged_tile_count(job.image, window.route) {
...
fn execute_staged_window_tiles<T: Copy + Send>(
    workers: &mut [BatchWorker],
    staged: &mut CpuStagedWorkspace,
    flattened: &mut CpuGroupFastWorkspace,
    window: &StagedWindowContext<'_>,
...
fn materialize_staged_window<T: Copy + Send>(
    workers: &mut [BatchWorker],
    staged: &mut CpuStagedWorkspace,
    window: &StagedWindowContext<'_>,
    image_jobs: &mut [CpuTypedDecodeJob<'_, '_, T>],
