...
extern "C" __global__ void j2k_deinterleave_to_f32(
...
extern "C" __global__ void j2k_deinterleave_strided_to_f32(
    const unsigned char *pixels,
    float *components,
...
extern "C" __global__ void j2k_forward_rct(
    float *plane0,
    float *plane1,
    float *plane2,
...
extern "C" __global__ void j2k_forward_ict(
    float *plane0,
    float *plane1,
    float *plane2,
    unsigned long long len
...
__device__ float j2k_fdwt53_predict_row(
    const float *src,
...
__device__ float j2k_fdwt53_predict_col(
    const float *src,
    unsigned int x,
...
extern "C" __global__ void j2k_forward_dwt53_horizontal(
    const float *src,
    float *dst,
    unsigned int full_width,
    ...
    if (x < low_width) {
        ...
        const float left = x > 0u
            ? j2k_fdwt53_predict_row(src, row_base, current_width, x - 1u)
    ...
    dst[row_base + x] = j2k_fdwt53_predict_row(
        src,
        row_base,
        current_width,
        x - low_width
...
extern "C" __global__ void j2k_forward_dwt53_vertical(
    const float *src,
    float *dst,
    unsigned int full_width,
    unsigned int current_width,
    ...
    if (y < low_height) {
        ...
        const float top = y > 0u
            ? j2k_fdwt53_predict_col(src, x, full_width, current_height, y - 1u)
            : j2k_fdwt53_predict_col(src, x, full_width, current_height, 0u);
    ...
    dst[y * full_width + x] = j2k_fdwt53_predict_col(
        src,
        x,
        full_width,
        current_height,
...
__device__ float j2k_fdwt97_high1_row(
    const float *src,
    unsigned int row_base,
    unsigned int width,
    unsigned int high_index
...
__device__ float j2k_fdwt97_low1_row(
    const float *src,
    unsigned int row_base,
    unsigned int width,
    ...
    const float left = low_index > 0u
        ? j2k_fdwt97_high1_row(src, row_base, width, low_index - 1u)
        : j2k_fdwt97_high1_row(src, row_base, width, 0u);
...
__device__ float j2k_fdwt97_high2_row(
    const float *src,
    unsigned int row_base,
    unsigned int width,
    unsigned int high_index
    ...
    const float left = j2k_fdwt97_low1_row(src, row_base, width, high_index);
    const float right = (odd + 1u < width)
        ? j2k_fdwt97_low1_row(src, row_base, width, high_index + 1u)
        : j2k_fdwt97_low1_row(src, row_base, width, last_low);
    return j2k_fdwt97_high1_row(src, row_base, width, high_index)
        + J2K_FDWT97_GAMMA * (left + right);
}

__device__ float j2k_fdwt97_low2_row(
    const float *src,
    unsigned int row_base,
    unsigned int width,
    unsigned int low_index
    ...
    const unsigned int even = low_index * 2u;
    const float left = low_index > 0u
        ? j2k_fdwt97_high2_row(src, row_base, width, low_index - 1u)
        : j2k_fdwt97_high2_row(src, row_base, width, 0u);
    const float right = even + 1u < width
        ? j2k_fdwt97_high2_row(src, row_base, width, low_index)
        : left;
    return j2k_fdwt97_low1_row(src, row_base, width, low_index)
        + J2K_FDWT97_DELTA * (left + right);
}

extern "C" __global__ void j2k_forward_dwt97_horizontal(
    const float *src,
    float *dst,
    unsigned int full_width,
    unsigned int current_width,
    ...
    const unsigned int row_base = y * full_width;
    if (x < low_width) {
        dst[row_base + x] = j2k_fdwt97_low2_row(src, row_base, current_width, x)
            * J2K_FDWT97_INV_KAPPA;
        return;
    }

    dst[row_base + x] = j2k_fdwt97_high2_row(
        src,
        row_base,
        current_width,
        x - low_width
...
__device__ float j2k_fdwt97_high1_col(
    const float *src,
    unsigned int x,
    unsigned int full_width,
    unsigned int height,
...
__device__ float j2k_fdwt97_low1_col(
    const float *src,
    unsigned int x,
    unsigned int full_width,
    unsigned int height,
    ...
    const float top = low_index > 0u
        ? j2k_fdwt97_high1_col(src, x, full_width, height, low_index - 1u)
        : j2k_fdwt97_high1_col(src, x, full_width, height, 0u);
...
__device__ float j2k_fdwt97_high2_col(
    const float *src,
    unsigned int x,
    unsigned int full_width,
    unsigned int height,
    ...
    const unsigned int last_even = (height % 2u == 0u) ? height - 2u : height - 1u;
    const unsigned int last_low = last_even / 2u;
    const float top = j2k_fdwt97_low1_col(src, x, full_width, height, high_index);
    const float bottom = (odd + 1u < height)
        ? j2k_fdwt97_low1_col(src, x, full_width, height, high_index + 1u)
        : j2k_fdwt97_low1_col(src, x, full_width, height, last_low);
    return j2k_fdwt97_high1_col(src, x, full_width, height, high_index)
        + J2K_FDWT97_GAMMA * (top + bottom);
}

__device__ float j2k_fdwt97_low2_col(
    const float *src,
    unsigned int x,
    unsigned int full_width,
    unsigned int height,
    ...
    const unsigned int even = low_index * 2u;
    const float top = low_index > 0u
        ? j2k_fdwt97_high2_col(src, x, full_width, height, low_index - 1u)
        : j2k_fdwt97_high2_col(src, x, full_width, height, 0u);
    const float bottom = even + 1u < height
        ? j2k_fdwt97_high2_col(src, x, full_width, height, low_index)
        : top;
    return j2k_fdwt97_low1_col(src, x, full_width, height, low_index)
        + J2K_FDWT97_DELTA * (top + bottom);
}

extern "C" __global__ void j2k_forward_dwt97_vertical(
    const float *src,
    float *dst,
    unsigned int full_width,
    unsigned int current_width,
    ...
    if (y < low_height) {
        dst[y * full_width + x] =
           j2k_fdwt97_low2_col(src, x, full_width, current_height, y)
            * J2K_FDWT97_INV_KAPPA;
        return;
    }

    dst[y * full_width + x] = j2k_fdwt97_high2_col(
        src,
        x,
        full_width,
        current_height,
...
__device__ int j2k_quantize_sample(
    float sample,
...
extern "C" __global__ void j2k_quantize_subband(
    const float *samples,
    int *coefficients,
    unsigned long long len,
    unsigned int step_exponent,
    ...
    coefficients[idx] = j2k_quantize_sample(
        samples[idx],
        step_exponent,
        step_mantissa,
        range_bits,
...
extern "C" __global__ void j2k_quantize_subband_strided(
    const float *samples,
    int *coefficients,
    unsigned int x0,
    unsigned int y0,
    ...
    coefficients[output_index] = j2k_quantize_sample(
