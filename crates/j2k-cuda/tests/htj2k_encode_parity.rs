...
// `J2K_REQUIRE_CUDA_RUNTIME` is absent.  They run for real only on the
// CI CUDA runner.
...
// ---------------------------------------------------------------------------
//
// If CI sets J2K_REQUIRE_CUDA_RUNTIME but the `cuda-runtime` feature was
...
fn cuda_runtime_required_implies_feature_compiled() {
    ...
    let cuda_runtime_required = std::env::var_os("J2K_REQUIRE_CUDA_RUNTIME").is_some();
    let cuda_runtime_feature_enabled = cfg!(feature = "cuda-runtime");
    assert!(
        !cuda_runtime_required || cuda_runtime_feature_enabled,
        "J2K_REQUIRE_CUDA_RUNTIME is set but the cuda-runtime feature is not compiled — \
...
// (crates/j2k-cuda/src/encode.rs:88–91), which sets
// `backend = RequireDevice` and otherwise leaves all other fields untouched.
// It then delegates to `j2k::encode_j2k_lossless_with_accelerator`
// (encode.rs:40–45), which internally calls `native_lossless_options` to build
// the `EncodeOptions` for the native encoder (j2k/src/encode.rs:377–392).
//
// The CPU reference here is `j2k::encode_j2k_lossless(samples, &opts)`
...
// (decode_native_with_context in j2k-native/src/lib.rs), so signed
        ...
        for &depth in MATRIX_DEPTHS {
            for &signed in MATRIX_SIGNED {
                for &levels in MATRIX_LEVELS {
                    ...
                    // (j2k/src/encode.rs:341–355 via encode_cpu).
...
fn lossless_facade_in_scope_input_never_hits_ok_none_fallback() {
    ...
    use j2k_core::{BackendKind, CodecError as _};
    use j2k_cuda::encode_j2k_lossless_with_cuda;
