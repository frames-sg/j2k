...
use j2k_cuda::{Codec, CudaEncodeStageAccelerator, CudaSession, J2kDecoder, SurfaceResidency};
use j2k_test_support::{
    append_auto_routing_output as append_output, auto_routing_operation_label as operation_label,
    auto_routing_route_cell as route_cell, auto_routing_sha256, load_auto_routing_manifest,
    load_auto_routing_pnm, write_auto_routing_evidence, AutoRoutingBackend, AutoRoutingCell,
    AutoRoutingEvidence, AutoRoutingOperation, AutoRoutingPixelFormat, AutoRoutingPlatform,
    AutoRoutingPnm, AutoRoutingWorkload, AutoRoutingWorkloadKind,
};
...
fn main() {
    ...
    let workloads = load_auto_routing_manifest(&manifest_path, &corpus_root)
        .unwrap_or_else(|error| panic!("load CUDA Auto-routing workloads: {error}"));
        ...
        match workload.kind {
            AutoRoutingWorkloadKind::Decode => {
                ...
                for operation in [
                    ...
                    cells.push(bench_decode_cell(&mut criterion, &decode, operation));
                }
            }
            AutoRoutingWorkloadKind::Encode => {
                let encode = load_auto_routing_pnm(workload).unwrap_or_else(|error| {
                    panic!("load CUDA encode workload {}: {error}", workload.id)
                });
                for operation in [
    ...
    let evidence = AutoRoutingEvidence {
        ...
        backend: AutoRoutingBackend::Cuda,
    ...
    write_auto_routing_evidence(&evidence_path, &evidence)
        .unwrap_or_else(|error| panic!("write CUDA Auto-routing evidence: {error}"));
}
...
fn assert_output_parity(case_id: &str, operation: AutoRoutingOperation, cpu: &[u8], hybrid: &[u8]) {
    if cpu == hybrid {
        return;
    }
