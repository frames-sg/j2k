...
pub(crate) fn encode(
    ...
    {
        // The default block profile is tuned for the HTJ2K 4x1024 profile,
    ...
    cs.write_headers(&mut outfile, &[])
        .map_err(|_| "HTJ2K header write failed")?;
    ...
    for y in 0..height as usize {
        ...
        for (comp_idx, line) in lines.iter().enumerate() {
            cs.exchange(line, comp_idx as u32)
                .map_err(|_| "HTJ2K line exchange failed")?;
        }
    }

    cs.flush(&mut outfile).map_err(|_| "HTJ2K flush failed")?;
...
pub(crate) fn decode_native(
    ...
    cs.read_headers(&mut infile)
        .map_err(|_| "HTJ2K header read failed")?;
    ...
    if actual_components != num_components || actual_bit_depth != bit_depth {
        return Err("HTJ2K metadata mismatch");
    }
    ...
    cs.create(&mut infile)
        .map_err(|_| "HTJ2K codestream decode failed")?;
    ...
    for _ in 0..height_usize {
        ...
        for comp_idx in 0..u32::from(num_components) {
            let line = cs.pull(comp_idx).ok_or("HTJ2K line pull failed")?;
            if line.len() != width_usize {
                return Err("HTJ2K decoded line width mismatch");
            }
...
fn decode_input_sample(
    ...
    if bit_depth <= 8 {
        let raw = u32::from(
            *pixels
                .get(byte_offset)
                .ok_or("HTJ2K pixel data too short")?,
        );
