...
| Path | Pipeline |
| --- | --- |
| signinum | JPEG → (entropy decode, CPU) → DCT-grid 9/7 + quantize (GPU) → HT encode (CPU) → HTJ2K |
...
- **Wall-clock vs GPU-only** — signinum is a CPU/GPU hybrid; NVIDIA is all-GPU.
- **Per-stage breakdown** — signinum's pack/upload, IDCT+row-lift, column-lift,
  quantize, readback; NVIDIA's nvJPEG decode and nvJPEG2000 HT encode.
- **Output size + PSNR** — codestream bytes and reconstruction quality vs the
  nvJPEG-decoded source RGB.

Throughput uses the best of `ITERATIONS` runs after warmup. signinum runs through
the batch code-block path (`transcode_batch_with_accelerator`); NVIDIA runs
per-image — note this batching asymmetry when reading the numbers.
