...
fn extracted_wsi_jpegs_decode_when_local_corpus_is_available() {
    let require_corpus = std::env::var_os("J2K_REQUIRE_WSI_ROOT").is_some();
    let Some(root) = std::env::var_os("J2K_WSI_ROOT") else {
        assert!(
            !require_corpus,
            "J2K_REQUIRE_WSI_ROOT is set but J2K_WSI_ROOT is not configured"
        );
        return;
    };
    ...
    assert!(
        ...
        "J2K_REQUIRE_WSI_ROOT is set but J2K_WSI_ROOT did not contain any JPEG files"
    );
