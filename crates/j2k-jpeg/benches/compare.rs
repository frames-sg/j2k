...
use common::{
    ...
    j2k_decode_region_scaled, j2k_decode_reused, j2k_decode_rows, j2k_decode_scaled,
    j2k_decode_tile_batch_region_scaled, j2k_decode_tile_batch_scaled, j2k_decode_with_scratch,
    ...
    libjpeg_turbo_available, libjpeg_turbo_decode, libjpeg_turbo_decode_batch,
    libjpeg_turbo_decode_batch_region_scaled, libjpeg_turbo_decode_batch_scaled,
    libjpeg_turbo_decode_region, libjpeg_turbo_decode_region_scaled, libjpeg_turbo_decode_scaled,
    libjpeg_turbo_inspect, load_bench_inputs, output_geometry, should_bench_decode_rows_rgb,
    should_compare_full_frame, zune_decode, zune_decode_batch_region_scaled,
    zune_decode_batch_scaled, zune_decode_region, zune_decode_region_scaled, zune_decode_scaled,
    zune_inspect, DecodeMode, J2KTileBatchRegionScaledRgbSession, J2KTileBatchRgbOutputBuffers,
    J2KTileBatchRgbScratch, J2KTileBatchRgbSession, J2KTileBatchScaledRgbSession,
...
fn bench_compare(c: &mut Criterion) {
    ...
    for input in &inputs {
        inspect.bench_function(format!("j2k/{}", input.name), |b| {
            b.iter(|| j2k_inspect(&input.bytes));
        });
    ...
    for input in inputs.iter().filter(|input| {
        ...
        decode_rgb.bench_function(format!("j2k/{}", input.name), |b| {
            b.iter(|| j2k_decode(&input.bytes, DecodeMode::Rgb));
        });
    ...
    for input in inputs.iter().filter(|input| {
        ...
        decode_gray.bench_function(format!("j2k/{}", input.name), |b| {
            b.iter(|| j2k_decode(&input.bytes, DecodeMode::Gray));
        });
    ...
    for input in inputs.iter().filter(|input| {
        ...
        let dec = Decoder::new(&input.bytes).expect("j2k decoder (reused-setup)");
        let (fmt, stride, len) = output_geometry(&dec, DecodeMode::Rgb);
        let mut out = vec![0u8; len];
        decode_reused_rgb.bench_function(format!("j2k_reused/{}", input.name), |b| {
            b.iter(|| j2k_decode_reused(&dec, &mut out, stride, fmt));
        });
    }
    ...
    for input in inputs.iter().filter(|input| {
        ...
        let mut out = vec![0u8; len];
        decode_reused_gray.bench_function(format!("j2k_reused/{}", input.name), |b| {
            b.iter(|| j2k_decode_reused(&dec, &mut out, stride, fmt));
        });
    }
    ...
    for input in inputs.iter().filter(|input| {
        ...
        let dec = Decoder::new(&input.bytes).expect("j2k decoder (scratch-setup)");
        let (fmt, stride, len) = output_geometry(&dec, DecodeMode::Rgb);
        let mut out = vec![0u8; len];
        let mut pool = ScratchPool::new();
        // Warm the pool once so iteration 1 pays zero allocation cost.
        j2k_decode_with_scratch(&dec, &mut pool, &mut out, stride, fmt);
        decode_scratch_rgb.bench_function(format!("j2k_scratch/{}", input.name), |b| {
            b.iter(|| j2k_decode_with_scratch(&dec, &mut pool, &mut out, stride, fmt));
        });
    }
    ...
    for input in inputs.iter().filter(|input| {
        ...
        let mut pool = ScratchPool::new();
        j2k_decode_with_scratch(&dec, &mut pool, &mut out, stride, fmt);
        decode_scratch_gray.bench_function(format!("j2k_scratch/{}", input.name), |b| {
            b.iter(|| j2k_decode_with_scratch(&dec, &mut pool, &mut out, stride, fmt));
        });
    }
    decode_scratch_gray.finish();

    // CPU-first JPEG proving groups start here. These large tiled image benches are
    ...
    {
        decode_rows_rgb.bench_function(format!("j2k/{}", input.name), |b| {
            b.iter(|| j2k_decode_rows(&input.bytes));
        });
    }
    ...
    for input in inputs.iter().filter(|input| input.mode == DecodeMode::Rgb) {
        ...
        let mut j2k_batch = J2KTileBatchRgbScratch::new(bytes, batch_size);
        wsi_tile_batch_rgb.bench_function(format!("j2k/{}", input.name), move |b| {
            b.iter(|| j2k_batch.run(bytes));
        });
        if libjpeg_turbo_available() {
            let mut turbo = TurboJpegDecoder::new().expect("libjpeg-turbo decoder");
    ...
    for batch_size in [16usize, 64, 256] {
        for input in inputs.iter().filter(|input| input.mode == DecodeMode::Rgb) {
            let bytes = &input.bytes;
            let mut current_batch = J2KTileBatchRgbScratch::new(bytes, batch_size);
            ...
            for workers in [1usize, 2, 4] {
                ...
                let mut fixed_worker_batch =
                    J2KTileBatchRgbScratch::new_with_options(bytes, batch_size, options);
                wsi_tile_batch_session_rgb.bench_function(
                    format!(
            ...
            let bytes = &input.bytes;
            let mut session_batch = J2KTileBatchRgbSession::new(bytes, batch_size);
            wsi_tile_batch_session_rgb.bench_function(
            ...
            let bytes = &input.bytes;
            let mut output_session = J2KTileBatchRgbOutputBuffers::new(bytes, batch_size);
            wsi_tile_batch_session_rgb.bench_function(
            ...
            wsi_tile_batch_session_rgb.bench_function(
                ...
                move |b| {
                    b.iter(|| {
                        j2k_decode_tile_batch_scaled(bytes, batch_size, Downscale::Quarter);
                    });
                },
            );
            ...
            let mut scaled_session =
                J2KTileBatchScaledRgbSession::new(bytes, batch_size, Downscale::Quarter);
            wsi_tile_batch_session_rgb.bench_function(
            ...
            let bytes = &input.bytes;
            let mut region_scaled_session =
                J2KTileBatchRegionScaledRgbSession::new(bytes, batch_size, 256, Downscale::Quarter);
            wsi_tile_batch_session_rgb.bench_function(
                format!(
    ...
    for input in inputs.iter().filter(|input| {
        ...
        wsi_region_rgb.bench_function(format!("j2k/{}", input.name), |b| {
            b.iter(|| j2k_decode_region(&input.bytes, 256));
        });
    ...
    for (group_name, scale) in [
        ...
        for input in inputs.iter().filter(|input| {
            ...
            group.bench_function(format!("j2k/{}", input.name), |b| {
                b.iter(|| j2k_decode_scaled(&input.bytes, scale));
            });
    ...
    for (group_name, scale) in [
        ...
        for input in inputs.iter().filter(|input| {
            ...
            let roi = centered_roi(input.dimensions, 256);
            group.bench_function(format!("j2k/{}", input.name), |b| {
                b.iter(|| j2k_decode_region_scaled(&input.bytes, 256, scale));
            });
            group.bench_function(format!("jpeg-decoder/{}", input.name), |b| {
                b.iter(|| {
    ...
    for input in inputs.iter().filter(|input| input.mode == DecodeMode::Rgb) {
        wsi_tile_batch_scaled_rgb_q4.bench_function(format!("j2k/{}", input.name), |b| {
            b.iter(|| {
                j2k_decode_tile_batch_scaled(&input.bytes, 64, Downscale::Quarter);
            });
        });
    ...
    for input in inputs.iter().filter(|input| input.mode == DecodeMode::Rgb) {
        let roi = centered_roi(input.dimensions, 256);
        wsi_tile_batch_region_scaled_rgb_q4.bench_function(format!("j2k/{}", input.name), |b| {
            b.iter(|| {
                j2k_decode_tile_batch_region_scaled(&input.bytes, 64, 256, Downscale::Quarter);
            });
        });
...
fn wsi_tile_batch_size() -> usize {
    std::env::var("J2K_JPEG_TILE_BATCH_SIZE")
        .ok()
