...
//! Decode a JPEG through `j2k-jpeg` and write a viewable 24-bit BMP.
//!
//! Run with:
//! `cargo run -p j2k-jpeg --example dump_bmp`
//!
//! Or pass an input and output path:
//! `cargo run -p j2k-jpeg --example dump_bmp -- input.jpg output.bmp`
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (input_label, bytes, output_path) = match args.as_slice() {
        [] => (
        ...
        _ => {
            return Err(
                "usage: cargo run -p j2k-jpeg --example dump_bmp -- [input.jpg output.bmp]".into(),
            )
        }
    };
