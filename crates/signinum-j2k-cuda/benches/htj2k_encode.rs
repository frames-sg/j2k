...
fn cuda_encode_available(
    ...
    match public_result {
        ...
        Ok(_) if std::env::var_os("SIGNINUM_REQUIRE_CUDA_BENCH").is_some() => {
            panic!("SIGNINUM_REQUIRE_CUDA_BENCH is set but CUDA HTJ2K encode was not resident")
        }
        Ok(_) => {
        ...
        Err(error) if std::env::var_os("SIGNINUM_REQUIRE_CUDA_BENCH").is_some() => {
            panic!("SIGNINUM_REQUIRE_CUDA_BENCH is set but CUDA HTJ2K encode failed: {error}")
        }
        Err(error) => {
    ...
    let context = match CudaContext::system_default() {
        Ok(context) => context,
        Err(error) if std::env::var_os("SIGNINUM_REQUIRE_CUDA_BENCH").is_some() => {
            panic!("SIGNINUM_REQUIRE_CUDA_BENCH is set but CUDA context failed: {error}")
        }
        Err(error) => {
            eprintln!("skipping CUDA HTJ2K encode benches: {error}");
            return false;
    ...
    let resources = match context.upload_htj2k_encode_resources(cuda_encode_tables(&uvlc_table)) {
        ...
        Err(error) if std::env::var_os("SIGNINUM_REQUIRE_CUDA_BENCH").is_some() => {
            panic!("SIGNINUM_REQUIRE_CUDA_BENCH is set but CUDA HTJ2K encode resource upload failed: {error}")
        }
        Err(error) => {
            eprintln!("skipping CUDA HTJ2K encode benches: {error}");
            return false;
    ...
    match result {
        ...
        Ok(_) if std::env::var_os("SIGNINUM_REQUIRE_CUDA_BENCH").is_some() => {
            panic!("SIGNINUM_REQUIRE_CUDA_BENCH is set but CUDA HTJ2K code-block encode did not dispatch")
        }
        Ok(_) => {
        ...
        Err(error) if std::env::var_os("SIGNINUM_REQUIRE_CUDA_BENCH").is_some() => {
            panic!("SIGNINUM_REQUIRE_CUDA_BENCH is set but CUDA HTJ2K code-block encode failed: {error}")
        }
        Err(error) => {
            eprintln!("skipping CUDA HTJ2K encode benches: {error}");
            false
