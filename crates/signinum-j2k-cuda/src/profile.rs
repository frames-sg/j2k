...
const CUDA_TRACE_ENV_VAR: &str = "SIGNINUM_J2K_CUDA_TRACE";
...
impl CudaHtj2kProfileReport {
    /// Emit the report using `SIGNINUM_J2K_PROFILE_STAGES`, when enabled.
...
fn export_trace_if_requested(path: &str, report: &CudaHtj2kProfileReport) {
    ...
    if let Err(error) = std::fs::write(&trace_path, trace) {
        std::eprintln!("signinum_profile codec=j2k op=cuda_htj2k_trace path=cuda error={error}");
    }
}
...
fn export_encode_trace_if_requested(path: &str, report: &CudaHtj2kEncodeProfileReport) {
    ...
    if let Err(error) = std::fs::write(&trace_path, trace) {
        std::eprintln!(
            "signinum_profile codec=j2k op=cuda_htj2k_encode_trace path=cuda error={error}"
        );
    }
}
