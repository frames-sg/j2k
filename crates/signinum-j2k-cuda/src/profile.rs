...
mod tests {
    ...
    fn profile_row_uses_compact_key_value_format() {
        let row = format_profile_row(
            ...
            "gpu_route",
            "cuda",
            &[("request", "Cuda"), ("decision", "cuda_upload")],
        );
        assert_eq!(
            row,
            "signinum_profile codec=j2k op=gpu_route path=cuda request=Cuda decision=cuda_upload"
        );
    }
    ...
    fn profile_summary_counts_route_decisions() {
        ...
        assert_eq!(
            summary.format_rows(),
            vec![
                "signinum_profile_summary codec=j2k op=gpu_route path=cuda route_op=wrap_surface request=Cuda fmt=Rgb8 decision=cuda_upload count=2"
            ]
        );
    }
}
