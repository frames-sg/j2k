...
use j2k_jpeg::{ColorSpace, Decoder as CpuDecoder, ScratchPool};
...
fn main() {
    ...
    if inputs.is_empty() {
        eprintln!(
            "viewport_report: no eligible JPEG inputs found; set J2K_BENCH_INPUTS to extracted JPEG tiles or levels"
        );
        std::process::exit(2);
    }
