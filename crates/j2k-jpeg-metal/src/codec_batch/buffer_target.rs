...
//! Caller-owned Metal buffer batch validation, resizing, and submission.

use super::request::{
    MetalBufferBatchTarget, Rgb8MetalBatchOp, Rgb8MetalBatchRequest, Rgb8MetalBatchSource,
};
use crate::{compute, Codec, Error, MetalBackendSession, MetalBatchOutputBuffer, Surface};
...
impl Codec {
    ...
    pub fn decode_rgb8_batch_into_buffer_with_session(
        ...
        let results = match request.op {
            Rgb8MetalBatchOp::Full => {
                compute::batch_entry::decode_full_rgb8_batch_into_output_with_session(
                    &plan.requests,
                    output,
                    session,
                )?
            ...
            Rgb8MetalBatchOp::Scaled(_) | Rgb8MetalBatchOp::RegionScaled { .. } => {
                compute::batch_entry::decode_region_scaled_rgb8_batch_into_output_with_session(
                    &plan.requests,
                    output,
                    session,
                )?
    ...
    /// Decode a region-scaled RGB8 JPEG batch into a resizable Metal buffer.
