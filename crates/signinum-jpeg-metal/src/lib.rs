...
impl Codec {
    ...
    #[cfg(target_os = "macos")]
    /// Decode a full-tile RGB8 JPEG decoder batch into a reusable caller-owned Metal buffer.
    ///
    ...
    pub fn decode_rgb8_decoder_batch_into_metal_buffer_with_session(
        decoders: &[&Decoder<'_>],
        output: &MetalBatchOutputBuffer,
        session: &MetalBackendSession,
    ) -> Result<Vec<Result<Surface, Error>>, Error> {
        ...
        compute::decode_full_rgb8_batch_into_output_with_session(&plan.requests, output, session)?
            .ok_or(Error::UnsupportedMetalRequest {
    ...
    #[cfg(target_os = "macos")]
    /// Decode a full-tile RGB8 JPEG batch into a resizable caller-owned Metal buffer.
    pub fn decode_rgb8_batch_into_resizable_metal_buffer_with_session(
        inputs: &[&[u8]],
        ...
        let Rgb8MetalBatchRequests {
            ...
            (batch::BatchOp::Full, decoder.info().dimensions)
        })?
        else {
            return Ok(Vec::new());
        };
        output.ensure_rgb8_tiles(session, output_dimensions, inputs.len())?;
    ...
    #[cfg(target_os = "macos")]
    /// Decode a full-tile RGB8 JPEG decoder batch into reusable caller-owned Metal RGBA8 textures.
    ///
    /// This variant accepts already parsed `Decoder` wrappers and reuses their
    /// cached Metal fast-packet state when building the resident batch request.
    pub fn decode_rgb8_decoder_batch_into_metal_textures_with_session(
        decoders: &[&Decoder<'_>],
        output: &MetalBatchTextureOutput,
        session: &MetalBackendSession,
    ) -> Result<Vec<Result<MetalTextureTile, Error>>, Error> {
        ...
        compute::decode_full_rgb8_batch_into_textures_with_session(
            &plan.requests,
            output,
            session,
        )?
    ...
    #[cfg(target_os = "macos")]
    /// Decode a full-tile RGB8 JPEG batch into resizable caller-owned Metal RGBA8 textures.
    pub fn decode_rgb8_batch_into_resizable_metal_textures_with_session(
        inputs: &[&[u8]],
        ...
        output.ensure_rgba8_tiles(session, output_dimensions, inputs.len())?;
    ...
    #[cfg(target_os = "macos")]
    /// Decode a scaled RGB8 JPEG decoder batch into a reusable caller-owned Metal buffer.
    ///
    /// This variant accepts already parsed `Decoder` wrappers and reuses their
    /// cached Metal fast-packet state when building the resident batch request.
    pub fn decode_rgb8_decoder_scaled_batch_into_metal_buffer_with_session(
        decoders: &[&Decoder<'_>],
        scale: Downscale,
        output: &MetalBatchOutputBuffer,
        session: &MetalBackendSession,
    ...
    #[cfg(target_os = "macos")]
    /// Decode a scaled RGB8 JPEG batch into a resizable caller-owned Metal buffer.
    pub fn decode_rgb8_scaled_batch_into_resizable_metal_buffer_with_session(
        inputs: &[&[u8]],
        scale: Downscale,
        output: &mut MetalBatchOutputBuffer,
        session: &MetalBackendSession,
    ...
    #[cfg(target_os = "macos")]
    /// Decode a scaled RGB8 JPEG decoder batch into reusable caller-owned Metal RGBA8 textures.
    ///
    /// This variant accepts already parsed `Decoder` wrappers and reuses their
    /// cached Metal fast-packet state when building the resident batch request.
    pub fn decode_rgb8_decoder_scaled_batch_into_metal_textures_with_session(
        decoders: &[&Decoder<'_>],
        scale: Downscale,
        output: &MetalBatchTextureOutput,
        session: &MetalBackendSession,
    ...
    #[cfg(target_os = "macos")]
    /// Decode a scaled RGB8 JPEG batch into resizable caller-owned Metal RGBA8 textures.
    pub fn decode_rgb8_scaled_batch_into_resizable_metal_textures_with_session(
        inputs: &[&[u8]],
        scale: Downscale,
        output: &mut MetalBatchTextureOutput,
        session: &MetalBackendSession,
    ...
    #[cfg(target_os = "macos")]
    /// Decode a region-scaled RGB8 JPEG decoder batch into a reusable caller-owned Metal buffer.
    ///
    /// This variant accepts already parsed `Decoder` wrappers and reuses their
    /// cached Metal fast-packet state when building the resident batch request.
    pub fn decode_rgb8_decoder_region_scaled_batch_into_metal_buffer_with_session(
        decoders: &[&Decoder<'_>],
        roi: Rect,
        scale: Downscale,
        output: &MetalBatchOutputBuffer,
        ...
        .ok_or(Error::UnsupportedMetalRequest {
            reason: "JPEG Metal reusable decoder region-scaled batch output currently supports batchable RGB8 fast 4:2:0, 4:2:2, or 4:4:4 inputs with matching output shapes",
        })
    }

    #[cfg(target_os = "macos")]
    /// Decode a region-scaled RGB8 JPEG batch into a resizable caller-owned Metal buffer.
    pub fn decode_rgb8_region_scaled_batch_into_resizable_metal_buffer_with_session(
        ...
        let plan = Self::rgb8_metal_batch_requests_with_output_dimensions(inputs, |_| {
            (
                batch::BatchOp::RegionScaled { roi, scale },
                output_dimensions,
            )
    ...
    #[cfg(target_os = "macos")]
    /// Decode a region-scaled RGB8 JPEG decoder batch into a resizable caller-owned Metal buffer.
    ///
    /// This variant accepts already parsed `Decoder` wrappers and reuses their
    /// cached Metal fast-packet state when building the resident batch request.
    pub fn decode_rgb8_decoder_region_scaled_batch_into_resizable_metal_buffer_with_session(
        decoders: &[&Decoder<'_>],
        roi: Rect,
        scale: Downscale,
        output: &mut MetalBatchOutputBuffer,
    ...
    #[cfg(target_os = "macos")]
    /// Decode a region-scaled RGB8 JPEG decoder batch into reusable caller-owned Metal RGBA8 textures.
    ///
    /// This variant accepts already parsed `Decoder` wrappers and reuses their
    /// cached Metal fast-packet state when building the resident batch request.
    pub fn decode_rgb8_decoder_region_scaled_batch_into_metal_textures_with_session(
        decoders: &[&Decoder<'_>],
        roi: Rect,
        scale: Downscale,
        output: &MetalBatchTextureOutput,
        ...
        .ok_or(Error::UnsupportedMetalRequest {
            reason: "JPEG Metal texture decoder region-scaled batch output currently supports batchable RGB8 fast 4:2:0, 4:2:2, or 4:4:4 inputs with matching output shapes",
        })
    }

    #[cfg(target_os = "macos")]
    /// Decode a region-scaled RGB8 JPEG batch into resizable caller-owned Metal RGBA8 textures.
    ...
    #[cfg(target_os = "macos")]
    /// Decode a region-scaled RGB8 JPEG decoder batch into resizable caller-owned Metal RGBA8 textures.
    ///
    /// This variant accepts already parsed `Decoder` wrappers and reuses their
    /// cached Metal fast-packet state when building the resident batch request.
    pub fn decode_rgb8_decoder_region_scaled_batch_into_resizable_metal_textures_with_session(
        decoders: &[&Decoder<'_>],
        roi: Rect,
        scale: Downscale,
        output: &mut MetalBatchTextureOutput,
...
mod tests {
    ...
    fn rgb8_decoder_batch_can_write_into_fixed_metal_output_buffer() {
        ...
        let surfaces = Codec::decode_rgb8_decoder_batch_into_metal_buffer_with_session(
    ...
    fn rgb8_decoder_scaled_batch_can_write_into_fixed_metal_output_buffer() {
        ...
        let surfaces = Codec::decode_rgb8_decoder_scaled_batch_into_metal_buffer_with_session(
    ...
    fn rgb8_decoder_batch_can_write_into_fixed_metal_textures() {
        ...
        let tiles = Codec::decode_rgb8_decoder_batch_into_metal_textures_with_session(
            &decoders, &output, &session,
        )
    ...
    fn rgb8_scaled_batch_decode_resizes_reusable_metal_textures() {
        ...
        let tiles = Codec::decode_rgb8_scaled_batch_into_resizable_metal_textures_with_session(
            &inputs,
            scale,
            &mut output,
            &session,
    ...
    fn rgb8_decoder_scaled_batch_can_write_into_fixed_metal_textures() {
        ...
        let tiles = Codec::decode_rgb8_decoder_scaled_batch_into_metal_textures_with_session(
            &decoders, scale, &output, &session,
        )
    ...
    fn rgb8_table_mixed_fast422_region_scaled_texture_batch_groups_resident_dispatches() {
        ...
        let packet_a = build_metal_fast422_packet(&jpeg_a.data).expect("first fast422 packet");
        let packet_b = build_metal_fast422_packet(&jpeg_b.data).expect("second fast422 packet");
        let packet_c = build_metal_fast422_packet(&jpeg_c.data).expect("third fast422 packet");
        assert_eq!(packet_a.y_quant, packet_c.y_quant);
    ...
    fn rgb8_table_mixed_fast444_region_scaled_texture_batch_groups_resident_dispatches() {
        ...
        let packet_a = build_metal_fast444_packet(&jpeg_a.data).expect("first fast444 packet");
        let packet_b = build_metal_fast444_packet(&jpeg_b.data).expect("second fast444 packet");
        let packet_c = build_metal_fast444_packet(&jpeg_c.data).expect("third fast444 packet");
        assert_eq!(packet_a.y_quant, packet_c.y_quant);
        assert_eq!(packet_a.cb_quant, packet_c.cb_quant);
        assert_eq!(packet_a.cr_quant, packet_c.cr_quant);
        assert_eq!(packet_a.y_dc_table, packet_c.y_dc_table);
    ...
    fn rgb8_decoder_region_scaled_batch_resizes_reusable_metal_textures() {
        ...
        let tiles = Codec::decode_rgb8_decoder_region_scaled_batch_into_resizable_metal_textures_with_session(
            &decoders,
            roi,
            scale,
            &mut output,
