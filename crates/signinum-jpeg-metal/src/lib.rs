...
impl Codec {
    ...
    #[cfg(target_os = "macos")]
    fn rgb8_batch_jpeg_decode_op(op: Rgb8MetalBatchOp) -> signinum_jpeg::JpegDecodeOp {
        match op {
            Rgb8MetalBatchOp::Full => signinum_jpeg::JpegDecodeOp::Full,
            Rgb8MetalBatchOp::Scaled(scale) => signinum_jpeg::JpegDecodeOp::Scaled(scale),
            Rgb8MetalBatchOp::RegionScaled { roi, scale } => {
...
mod tests {
    ...
    fn rgb8_scaled_batch_decode_can_write_into_reusable_metal_output_buffer() {
        ...
        let surfaces = decode_rgb8_scaled_batch_into_metal_buffer_with_session(
    ...
    fn rgb8_decoder_batch_can_write_into_fixed_metal_textures() {
        ...
        let tiles = decode_rgb8_decoder_batch_into_metal_textures_with_session(
    ...
    fn rgb8_decoder_region_scaled_batch_resizes_reusable_metal_textures() {
        ...
        let tiles =
            decode_rgb8_decoder_region_scaled_batch_into_resizable_metal_textures_with_session(
                &decoders,
                roi,
                scale,
                &mut output,
    ...
    fn rgb8_texture_batch_decode_avoids_private_rgba_staging_buffers() {
        ...
        for (input, dimensions, expected_private_allocations) in cases {
            ...
            compute::reset_jpeg_private_buffer_allocations_for_test();
            let tiles =
                decode_rgb8_batch_into_metal_textures_with_session(&inputs, &output, &session)
                    .expect("decode into reusable textures");
            assert_eq!(tiles.len(), 2);
            for tile in tiles {
                assert_eq!(
