...
impl MetalRuntime {
    ...
    pub(crate) fn new_with_device(device: Device) -> Result<Self, String> {
        ...
        let pack_420_windowed_rgba_texture_function =
            library.get_function("jpeg_pack_420_windowed_rgba_texture", None)?;
        let pack_420_windowed_rgba_texture_pipeline = device
            .new_compute_pipeline_state_with_function(&pack_420_windowed_rgba_texture_function)?;
        ...
        let pack_422_windowed_rgba_texture_function =
            library.get_function("jpeg_pack_422_windowed_rgba_texture", None)?;
        let pack_422_windowed_rgba_texture_pipeline = device
            .new_compute_pipeline_state_with_function(&pack_422_windowed_rgba_texture_function)?;
        ...
        let fast420_rgba_texture_batch_decode_function =
            library.get_function("jpeg_decode_fast420_rgba_texture_batch", None)?;
        let fast420_rgba_texture_batch_decode_pipeline = device
            .new_compute_pipeline_state_with_function(
        ...
        let fast422_rgba_texture_batch_decode_function =
            library.get_function("jpeg_decode_fast422_rgba_texture_batch", None)?;
        let fast422_rgba_texture_batch_decode_pipeline = device
            .new_compute_pipeline_state_with_function(
                &fast422_rgba_texture_batch_decode_function,
            )?;
        let fast422_rgba_texture_boundary_function =
            library.get_function("jpeg_resolve_fast422_rgba_texture_boundaries", None)?;
        let fast422_rgba_texture_boundary_pipeline = device
            .new_compute_pipeline_state_with_function(&fast422_rgba_texture_boundary_function)?;
        ...
        let fast444_rgba_texture_batch_decode_function =
            library.get_function("jpeg_decode_fast444_rgba_texture_batch", None)?;
        let fast444_rgba_texture_batch_decode_pipeline = device
            .new_compute_pipeline_state_with_function(
                &fast444_rgba_texture_batch_decode_function,
            )?;
        let rgb8_to_rgba_texture_function =
            library.get_function("jpeg_copy_rgb8_to_rgba_texture", None)?;
        let rgb8_to_rgba_texture_pipeline =
            device.new_compute_pipeline_state_with_function(&rgb8_to_rgba_texture_function)?;
        ...
        Ok(Self {
            ...
            pack_420_windowed_rgba_texture_pipeline,
            ...
            pack_422_windowed_rgba_texture_pipeline,
            ...
            fast422_rgba_texture_batch_decode_pipeline,
            fast422_rgba_texture_boundary_pipeline,
            ...
            fast444_rgba_texture_batch_decode_pipeline,
            rgb8_to_rgba_texture_pipeline,
...
#[cfg(target_os = "macos")]
fn fast420_full_rgb_batch_groups(packets: &[&JpegMetalFast420PacketV1]) -> Option<Vec<Vec<usize>>> {
    ...
    'packet: for (index, packet) in packets.iter().copied().enumerate() {
        ...
        for group in &mut groups {
            ...
            if fast420_packets_share_full_rgb_batch_shape(
                first,
                packet,
...
#[cfg(target_os = "macos")]
struct Fast420RegionScaledGroup {
    indices: Vec<usize>,
    scale: signinum_core::Downscale,
...
#[cfg(target_os = "macos")]
fn fast420_region_scaled_batch_groups(
    requests: &[batch::QueuedRequest],
    packets: &[&JpegMetalFast420PacketV1],
) -> Result<Option<Vec<Vec<usize>>>, Error> {
    let mut groups = Vec::<Fast420RegionScaledGroup>::new();
        ...
        requests.iter().zip(packets.iter().copied()).enumerate()
    {
        ...
        groups.push(Fast420RegionScaledGroup {
            indices: vec![index],
            scale,
            plan,
        });
...
#[cfg(target_os = "macos")]
struct Fast422RegionScaledGroup {
    indices: Vec<usize>,
    scale: signinum_core::Downscale,
    plan: RegionScaledBatchPlan,
}
...
#[cfg(target_os = "macos")]
fn fast422_region_scaled_batch_groups(
    requests: &[batch::QueuedRequest],
    packets: &[&JpegMetalFast422PacketV1],
) -> Result<Option<Vec<Vec<usize>>>, Error> {
    let mut groups = Vec::<Fast422RegionScaledGroup>::new();
    'packet: for (index, (request, packet)) in
        requests.iter().zip(packets.iter().copied()).enumerate()
    {
        if packet.restart_interval_mcus != 0
        ...
        let segment_count_u32 = checked_u32(
            segment_count,
            "fast422 region scaled texture batch segment count",
        )?;
        ...
        for group in &mut groups {
            ...
            if scale == group.scale
                && plan == group.plan
                && fast422_packets_share_full_rgb_batch_shape(first, packet, first_segment_count)
            {
                group.indices.push(index);
                continue 'packet;
            }
        }
        groups.push(Fast422RegionScaledGroup {
            indices: vec![index],
            scale,
            plan,
        });
...
fn try_decode_fast420_full_rgb_batch_to_surfaces_with_mode_and_output(
    ...
    let Some(groups) = fast420_full_rgb_batch_groups(&fast420_packets) else {
        return Ok(None);
    };
    if groups.len() > 1 {
        return try_decode_grouped_fast420_full_rgb_batch_to_surfaces_with_output(
            runtime,
            requests,
            &fast420_packets,
            decode_mode,
...
#[cfg(target_os = "macos")]
fn try_decode_grouped_fast420_full_rgb_batch_to_surfaces_with_output(
    runtime: &MetalRuntime,
    requests: &[batch::QueuedRequest],
    ...
    for group_indices in groups {
        ...
        if let Some(output) = output {
            ...
            let packet = fast420_packets[first_group_index];
            let out_stride = packet.dimensions.0 as usize * PixelFormat::Rgb8.bytes_per_pixel();
            let out_tile_len = out_stride * packet.dimensions.1 as usize;
            ...
            if group_results.len() != group_indices.len() {
                return Err(Error::MetalKernel {
                    message: "JPEG Metal grouped fast420 buffer result count mismatch".to_string(),
                });
            }
    ...
    for (index, result) in merged_results.into_iter().enumerate() {
        results.push(result.ok_or_else(|| Error::MetalKernel {
            message: format!(
                "JPEG Metal grouped fast420 buffer result for tile {index} was missing"
            ),
        })?);
    }
    Ok(Some(results))
...
fn try_decode_fast420_full_rgba_batch_to_textures(
    ...
    if groups.len() > 1 {
        return try_decode_grouped_fast420_full_rgba_batch_to_textures(
            runtime,
            requests,
            &fast420_packets,
            output,
    ...
    if decode_mode == Fast420BatchDecodeMode::Fused {
        ...
        for index in 0..tile_count {
            ...
            let decoder_encoder = command_buffer.new_compute_command_encoder();
            decoder_encoder
                .set_compute_pipeline_state(&runtime.fast420_rgba_texture_batch_decode_pipeline);
...
#[cfg(target_os = "macos")]
fn try_decode_grouped_fast420_full_rgba_batch_to_textures(
    runtime: &MetalRuntime,
    requests: &[batch::QueuedRequest],
    fast420_packets: &[&JpegMetalFast420PacketV1],
    output: &crate::MetalBatchTextureOutput,
    ...
    for group_indices in groups {
        ...
        let Some(group_results) = try_decode_fast420_full_rgba_batch_to_textures(
            runtime,
            &group_requests,
            &group_packets,
            &group_output,
        ...
        if group_results.len() != group_indices.len() {
            return Err(Error::MetalKernel {
                message: "JPEG Metal grouped fast420 texture result count mismatch".to_string(),
            });
        }
        for (original_index, result) in group_indices.into_iter().zip(group_results) {
            merged_results[original_index] = Some(result);
    ...
    for (index, result) in merged_results.into_iter().enumerate() {
        results.push(result.ok_or_else(|| Error::MetalKernel {
            message: format!(
                "JPEG Metal grouped fast420 texture result for tile {index} was missing"
            ),
        })?);
    }
    Ok(Some(results))
...
fn try_decode_fast422_full_rgb_batch_to_surfaces_with_output(
    ...
    if groups.len() > 1 {
        return try_decode_grouped_fast422_full_rgb_batch_to_surfaces_with_output(
            runtime,
            requests,
            &fast422_packets,
            output,
...
#[cfg(target_os = "macos")]
fn try_decode_grouped_fast422_full_rgb_batch_to_surfaces_with_output(
    runtime: &MetalRuntime,
    requests: &[batch::QueuedRequest],
    ...
    for group_indices in groups {
        ...
        let Some(group_results) = try_decode_fast422_full_rgb_batch_to_surfaces_with_output(
            runtime,
            &group_requests,
            &group_packets,
            None,
        ...
        if let Some(output) = output {
            ...
            let packet = fast422_packets[first_group_index];
            let out_stride = packet.dimensions.0 as usize * PixelFormat::Rgb8.bytes_per_pixel();
            let out_tile_len = out_stride * packet.dimensions.1 as usize;
            for (original_index, result) in copy_grouped_surfaces_to_output(
                runtime,
            ...
            if group_results.len() != group_indices.len() {
                return Err(Error::MetalKernel {
                    message: "JPEG Metal grouped fast422 buffer result count mismatch".to_string(),
                });
            }
            for (original_index, result) in group_indices.into_iter().zip(group_results) {
                merged_results[original_index] = Some(result);
    ...
    for (index, result) in merged_results.into_iter().enumerate() {
        results.push(result.ok_or_else(|| Error::MetalKernel {
            message: format!(
                "JPEG Metal grouped fast422 buffer result for tile {index} was missing"
            ),
        })?);
    }
    Ok(Some(results))
...
fn try_decode_fast422_full_rgba_batch_to_textures(
    ...
    // H2V1 reconstruction needs neighboring chroma samples at horizontal MCU boundaries. The
    // fused path carries same-segment boundaries in-thread and resolves cross-segment boundaries
    // from compact shared records before returning the caller-owned texture.
    if segment_count > 0 {
        let statuses = vec![JpegDecodeStatus::default(); total_decode_threads as usize];
        let status_buffer = batch_scratch.shared_buffer_with_slice(
            &runtime.device,
        ...
        let boundary_meta =
            vec![0u32; total_decode_threads as usize * FAST422_TEXTURE_BOUNDARY_META_WORDS];
        let boundary_samples =
            vec![0u8; total_decode_threads as usize * FAST422_TEXTURE_BOUNDARY_SAMPLE_BYTES];
        let boundary_meta_buffer = batch_scratch.shared_buffer_with_slice(
            &runtime.device,
            "fast422_texture_boundary_meta",
            &boundary_meta,
        );
        let boundary_samples_buffer = batch_scratch.shared_buffer_with_bytes(
            &runtime.device,
            "fast422_texture_boundary_samples",
            &boundary_samples,
        );
        let dc_tables = [
            PreparedHuffmanHost::from(&first.y_dc_table),
        ...
        for index in 0..tile_count {
            ...
            let decode_params = JpegFast422TextureBatchParams {
                width,
                height,
                chroma_width,
                chroma_height,
                ...
                mcu_rows: first.mcu_rows,
                segment_count: segment_count_u32,
                tile_index: checked_u32(index, "fast422 texture batch tile index")?,
                alpha: u32::from(u8::MAX),
            };
            let decoder_encoder = command_buffer.new_compute_command_encoder();
            decoder_encoder
                .set_compute_pipeline_state(&runtime.fast422_rgba_texture_batch_decode_pipeline);
            decoder_encoder.set_buffer(0, Some(&entropy_buffers.payload), 0);
            decoder_encoder.set_bytes(
                4,
                size_of::<JpegFast422TextureBatchParams>() as u64,
                (&raw const decode_params).cast(),
            );
            decoder_encoder.set_bytes(
                5,
        ...
        if segment_count > 1 {
            for index in 0..tile_count {
                ...
                let boundary_encoder = command_buffer.new_compute_command_encoder();
                boundary_encoder
                    .set_compute_pipeline_state(&runtime.fast422_rgba_texture_boundary_pipeline);
                boundary_encoder.set_buffer(0, Some(&boundary_meta_buffer), 0);
                boundary_encoder.set_buffer(1, Some(&boundary_samples_buffer), 0);
                boundary_encoder.set_bytes(
                    2,
...
#[cfg(target_os = "macos")]
fn try_decode_fast420_restart_region_scaled_rgb_batch_to_surfaces_with_output(
    runtime: &MetalRuntime,
    requests: &[batch::QueuedRequest],
    fast420_packets: &[&JpegMetalFast420PacketV1],
    output: Option<&crate::MetalBatchOutputBuffer>,
    ...
    if output.is_some() {
        for (request, packet) in requests.iter().zip(fast420_packets.iter().copied()) {
            ...
            let segment_count_u32 = checked_u32(
                packet.entropy_checkpoints.len(),
                "fast420 restart region scaled buffer segment count",
            )?;
            let Some(plan) =
                fast420_region_scaled_batch_plan(packet, roi, scale, 1, segment_count_u32)
            else {
                return Ok(None);
            };
            batch_output_buffer_or_new(
    ...
    for (request, packet) in requests.iter().zip(fast420_packets.iter().copied()) {
        let decoder = CpuDecoder::new(request.input.as_ref())?;
        let batched_packet = BatchedFastPacket::Fast420(packet);
        results.push(decode_region_scaled_packet_surface(
            runtime,
            &decoder,
            request,
    ...
    for (index, result) in merged_results.into_iter().enumerate() {
        results.push(result.ok_or_else(|| Error::MetalKernel {
            message: format!(
                "JPEG Metal restart fast420 region scaled buffer result for tile {index} was missing"
            ),
        })?);
    }
    Ok(Some(results))
...
fn try_decode_fast420_region_scaled_rgb_batch_to_surfaces_with_output(
    ...
    if fast420_packets
        ...
        .any(|packet| packet.restart_interval_mcus != 0)
    {
        return try_decode_fast420_restart_region_scaled_rgb_batch_to_surfaces_with_output(
            runtime,
            requests,
            &fast420_packets,
            output,
    ...
    let Some(groups) = fast420_region_scaled_batch_groups(requests, &fast420_packets)? else {
        return Ok(None);
    };
    if groups.len() > 1 {
        return try_decode_grouped_fast420_region_scaled_rgb_batch_to_surfaces_with_output(
            runtime,
            requests,
            &fast420_packets,
            output,
...
#[cfg(target_os = "macos")]
fn try_decode_grouped_fast420_region_scaled_rgb_batch_to_surfaces_with_output(
    runtime: &MetalRuntime,
    requests: &[batch::QueuedRequest],
    fast420_packets: &[&JpegMetalFast420PacketV1],
    output: Option<&crate::MetalBatchOutputBuffer>,
    ...
    if let Some(output) = output {
        for (request, packet) in requests.iter().zip(fast420_packets.iter().copied()) {
            ...
            let segment_count_u32 = checked_u32(
                packet.entropy_checkpoints.len(),
                "fast420 grouped region scaled buffer segment count",
            )?;
            let Some(plan) =
                fast420_region_scaled_batch_plan(packet, roi, scale, 1, segment_count_u32)
            else {
    ...
    for group_indices in groups {
        ...
        if let Some(output) = output {
            ...
            if group_results.len() != group_indices.len() {
                return Err(Error::MetalKernel {
                    message:
                        "JPEG Metal grouped fast420 region scaled buffer result count mismatch"
                            .to_string(),
                });
            }
            for (original_index, result) in group_indices.into_iter().zip(group_results) {
    ...
    for (index, result) in merged_results.into_iter().enumerate() {
        results.push(result.ok_or_else(|| Error::MetalKernel {
            message: format!(
                "JPEG Metal grouped fast420 region scaled buffer result for tile {index} was missing"
            ),
        })?);
    }
    Ok(Some(results))
...
#[cfg(target_os = "macos")]
fn try_decode_fast420_restart_region_scaled_rgba_batch_to_textures(
    runtime: &MetalRuntime,
    requests: &[batch::QueuedRequest],
    fast420_packets: &[&JpegMetalFast420PacketV1],
    output: &crate::MetalBatchTextureOutput,
    ...
    for (request, packet) in requests.iter().zip(fast420_packets.iter().copied()) {
        ...
        let segment_count_u32 = checked_u32(
            packet.entropy_checkpoints.len(),
            "fast420 restart region scaled texture segment count",
        )?;
        let Some(plan) = fast420_region_scaled_batch_plan(packet, roi, scale, 1, segment_count_u32)
        else {
            return Ok(None);
    ...
    for (index, result) in merged_results.into_iter().enumerate() {
        results.push(result.ok_or_else(|| Error::MetalKernel {
            message: format!(
                "JPEG Metal restart fast420 region scaled texture result for tile {index} was missing"
            ),
        })?);
    }
    Ok(Some(results))
...
fn try_decode_fast420_region_scaled_rgba_batch_to_textures(
    ...
    if fast420_packets
        ...
        .any(|packet| packet.restart_interval_mcus != 0)
    {
        return try_decode_fast420_restart_region_scaled_rgba_batch_to_textures(
            runtime,
            requests,
            &fast420_packets,
            output,
    ...
    let total_decode_threads = checked_u32(
        tile_count
            .checked_mul(segment_count)
            .ok_or_else(|| Error::MetalKernel {
                message: "JPEG Metal fast420 region scaled texture decode thread count overflowed"
                    .to_string(),
            })?,
        "fast420 region scaled texture decode thread count",
    )?;
    ...
    let Some(entropy_buffers) = batch_entropy_buffers(
        ...
        &mut batch_scratch,
        BatchEntropyBufferKeys {
            payload: "fast420_region_scaled_texture_entropy",
            offsets: "fast420_region_scaled_texture_entropy_offsets",
            lens: "fast420_region_scaled_texture_entropy_lens",
            checkpoints: "fast420_region_scaled_texture_entropy_checkpoints",
        },
        fast420_packets
            .iter()
            .map(|packet| packet.entropy_bytes.as_slice()),
    ...
    let y_plane = batch_scratch.private_buffer(
        &runtime.device,
        "fast420_region_scaled_texture_y",
        first_plan.y_len * tile_count,
    );
    let cb_plane = batch_scratch.private_buffer(
        &runtime.device,
        "fast420_region_scaled_texture_cb",
        first_plan.chroma_len * tile_count,
    );
    let cr_plane = batch_scratch.private_buffer(
        &runtime.device,
        "fast420_region_scaled_texture_cr",
        first_plan.chroma_len * tile_count,
    );
    let statuses = vec![JpegDecodeStatus::default(); total_decode_threads as usize];
    let status_buffer = batch_scratch.shared_buffer_with_slice(
        &runtime.device,
        "fast420_region_scaled_texture_status",
        &statuses,
    );
    let dc_tables = [
        PreparedHuffmanHost::from(&first.y_dc_table),
...
fn try_decode_fast422_region_scaled_rgb_batch_to_surfaces_with_output(
    ...
    let Some(groups) = fast422_region_scaled_batch_groups(requests, &fast422_packets)? else {
        return Ok(None);
    };
    if groups.len() > 1 {
        return try_decode_grouped_fast422_region_scaled_rgb_batch_to_surfaces_with_output(
            runtime,
            requests,
            &fast422_packets,
            output,
...
#[cfg(target_os = "macos")]
fn try_decode_grouped_fast422_region_scaled_rgb_batch_to_surfaces_with_output(
    runtime: &MetalRuntime,
    requests: &[batch::QueuedRequest],
    fast422_packets: &[&JpegMetalFast422PacketV1],
    output: Option<&crate::MetalBatchOutputBuffer>,
    ...
    if let Some(output) = output {
        for (request, packet) in requests.iter().zip(fast422_packets.iter().copied()) {
            ...
            let segment_count_u32 = checked_u32(
                packet.entropy_checkpoints.len(),
                "fast422 grouped region scaled buffer segment count",
            )?;
            let Some(plan) =
                fast422_region_scaled_batch_plan(packet, roi, scale, 1, segment_count_u32)
            else {
    ...
    for group_indices in groups {
        ...
        if let Some(output) = output {
            ...
            if group_results.len() != group_indices.len() {
                return Err(Error::MetalKernel {
                    message:
                        "JPEG Metal grouped fast422 region scaled buffer result count mismatch"
                            .to_string(),
                });
            }
            for (original_index, result) in group_indices.into_iter().zip(group_results) {
    ...
    for (index, result) in merged_results.into_iter().enumerate() {
        results.push(result.ok_or_else(|| Error::MetalKernel {
            message: format!(
                "JPEG Metal grouped fast422 region scaled buffer result for tile {index} was missing"
            ),
        })?);
    }
    Ok(Some(results))
...
fn try_decode_fast422_region_scaled_rgba_batch_to_textures(
    ...
    if groups.len() > 1 {
        return try_decode_grouped_fast422_region_scaled_rgba_batch_to_textures(
            runtime,
            requests,
            &fast422_packets,
            output,
    ...
    let total_decode_threads = checked_u32(
        tile_count
            .checked_mul(segment_count)
            .ok_or_else(|| Error::MetalKernel {
                message: "JPEG Metal fast422 region scaled texture decode thread count overflowed"
                    .to_string(),
            })?,
        "fast422 region scaled texture decode thread count",
    )?;
    ...
    let Some(entropy_buffers) = batch_entropy_buffers(
        ...
        &mut batch_scratch,
        BatchEntropyBufferKeys {
            payload: "fast422_region_scaled_texture_entropy",
            offsets: "fast422_region_scaled_texture_entropy_offsets",
            lens: "fast422_region_scaled_texture_entropy_lens",
            checkpoints: "fast422_region_scaled_texture_entropy_checkpoints",
        },
        fast422_packets
            .iter()
            .map(|packet| packet.entropy_bytes.as_slice()),
    ...
    let y_plane = batch_scratch.private_buffer(
        &runtime.device,
        "fast422_region_scaled_texture_y",
        first_plan.y_len * tile_count,
    );
    let cb_plane = batch_scratch.private_buffer(
        &runtime.device,
        "fast422_region_scaled_texture_cb",
        first_plan.chroma_len * tile_count,
    );
    let cr_plane = batch_scratch.private_buffer(
        &runtime.device,
        "fast422_region_scaled_texture_cr",
        first_plan.chroma_len * tile_count,
    );
    let statuses = vec![JpegDecodeStatus::default(); total_decode_threads as usize];
    let status_buffer = batch_scratch.shared_buffer_with_slice(
        &runtime.device,
        "fast422_region_scaled_texture_status",
        &statuses,
    );
    let dc_tables = [
        PreparedHuffmanHost::from(&first.y_dc_table),
...
#[cfg(target_os = "macos")]
fn try_decode_grouped_fast422_region_scaled_rgba_batch_to_textures(
    runtime: &MetalRuntime,
    requests: &[batch::QueuedRequest],
    fast422_packets: &[&JpegMetalFast422PacketV1],
    output: &crate::MetalBatchTextureOutput,
    ...
    for (request, packet) in requests.iter().zip(fast422_packets.iter().copied()) {
        ...
        let segment_count_u32 = checked_u32(
            packet.entropy_checkpoints.len(),
            "fast422 grouped region scaled texture batch segment count",
        )?;
        let Some(plan) = fast422_region_scaled_batch_plan(packet, roi, scale, 1, segment_count_u32)
        else {
            return Ok(None);
    ...
    for group_indices in groups {
        ...
        let Some(group_results) = try_decode_fast422_region_scaled_rgba_batch_to_textures(
            runtime,
            &group_requests,
            &group_packets,
            &group_output,
        ...
        if group_results.len() != group_indices.len() {
            return Err(Error::MetalKernel {
                message: "JPEG Metal grouped fast422 region scaled texture result count mismatch"
                    .to_string(),
            });
        }
        for (original_index, result) in group_indices.into_iter().zip(group_results) {
    ...
    for (index, result) in merged_results.into_iter().enumerate() {
        results.push(result.ok_or_else(|| Error::MetalKernel {
            message: format!(
                "JPEG Metal grouped fast422 region scaled texture result for tile {index} was missing"
            ),
        })?);
    }
    Ok(Some(results))
