...
and tile decompression APIs. Runtime backend selection defaults to `Auto`:
device paths are used for supported workloads only when Metal or CUDA features
are compiled and available, with CPU as the fallback. Metal and CUDA remain
available through the explicit `metal`, `cuda`, or `gpu` features.
