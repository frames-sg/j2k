...
const ZIGZAG: [u8; 64] = j2k_codec_math::jpeg::ZIGZAG;
...
#[inline(always)]
fn jpeg_encode_blocks_per_mcu(params: J2kJpegBaselineEncodeParams) -> u32 {
    let mut blocks = params.h0 * params.v0;
...
fn jpeg_encode_precompute_one_mcu(
    input: *const u8,
    coefficients: *mut i32,
    params: J2kJpegBaselineEncodeParams,
    local_mcu: u32,
    coefficient_block_base: u32,
    q_luma: *const u8,
    q_chroma: *const u8,
) {
    let blocks_per_mcu = jpeg_encode_blocks_per_mcu(params);
    let mcu_x = local_mcu % params.mcus_per_row;
    let mcu_y = local_mcu / params.mcus_per_row;
    ...
    while component < params.components {
        ...
        while block_y < v {
            ...
            while block_x < h {
                ...
                let mut block_coefficients = [0i32; 64];
                ...
                jpeg_encode_fdct_quantize(
                    &block,
                    if component == 0 { q_luma } else { q_chroma },
                    &mut block_coefficients,
                );
                let destination = (coefficient_block_base
                    + local_mcu * blocks_per_mcu
                    + block_index)
                    * 64;
                let mut index = 0;
                while index < 64 {
                    simt_store(
                        ...
                        block_coefficients[index as usize],
                    );
                    index += 1;
                }
                block_index += 1;
...
fn jpeg_encode_baseline_entropy_from_coeffs_one(
        ...
        while mcu_x < params.mcus_per_row {
            ...
            while component < params.components {
                ...
                while block_y < v {
                    ...
                    while block_x < h {
                        let mut block_coefficients = [0i32; 64];
                        let source = block_index * 64;
                        let mut index = 0;
                        while index < 64 {
                            block_coefficients[index as usize] =
                                simt_load(coefficients, (source + index) as usize);
                            index += 1;
                        }
                        block_index += 1;
...
mod kernels {
    ...
    #[kernel]
    pub unsafe fn j2k_jpeg_encode_baseline_precompute_batch(
        input: *const u8,
        coefficients: *mut i32,
        ...
        let raw_gid = thread::index_1d().get();
        if raw_gid >= total_mcus as usize {
            return;
        }
        let mut local_mcu = raw_gid as u32;
        let mut tile_index = 0;
        let mut coefficient_block_base = 0;
        while tile_index < tile_count {
            let tile_params = load_params(params, tile_index);
            let tile_mcus = tile_params.mcus_per_row * tile_params.mcu_rows;
            if local_mcu < tile_mcus {
                let tile_input = unsafe {
                    // SAFETY: Host validation proves the selected tile's input offset and row
                    // footprint remain inside the input allocation.
                    input.add(tile_params.input_offset_bytes as usize)
                };
                jpeg_encode_precompute_one_mcu(
                    tile_input,
                    coefficients,
                    tile_params,
                    local_mcu,
                    coefficient_block_base,
                    q_luma,
                    q_chroma,
                );
                return;
            ...
            local_mcu -= tile_mcus;
            coefficient_block_base += tile_mcus * jpeg_encode_blocks_per_mcu(tile_params);
            tile_index += 1;
        }
    }

    #[kernel]
    pub unsafe fn j2k_jpeg_encode_baseline_entropy_from_coeffs_batch(
        coefficients: *const i32,
        entropy: *mut u8,
        status: *mut J2kJpegBaselineEncodeStatus,
        params: *const J2kJpegBaselineEncodeParams,
        ...
        while prior_index < gid {
            let prior = load_params(params, prior_index);
        ...
        let tile_coefficients = unsafe {
            // SAFETY: The checked host staging plan allocates every prior tile's coefficient
            // span and the current tile span in this exact packed order.
            coefficients.add(coefficient_block_base as usize * 64)
        };
        let tile_entropy = unsafe {
            // SAFETY: Host validation proves this tile's disjoint entropy range is in bounds.
            entropy.add(tile_params.entropy_offset_bytes as usize)
        };
        let tile_status = unsafe {
            // SAFETY: The host allocates one status per tile and the guard proves `gid` is valid.
