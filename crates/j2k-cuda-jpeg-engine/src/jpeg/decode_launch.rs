...
use self::profiling::CudaJpegDecodeStageProfiler;
use super::{
...
unsafe impl j2k_cuda_runtime::CudaKernelParam for CudaJpegDecodeQuantPtrs {}
// SAFETY: these `#[repr(C)]` structs contain only CUDA device-pointer scalar
// values and mirror the pointer-only structs consumed by the CUDA Oxide kernels.
unsafe impl j2k_cuda_runtime::CudaKernelParam for CudaJpegDecodeHuffmanPtrs {}
...
impl JpegCudaEngine<'_> {
    ...
    pub(super) fn decode_jpeg_rgb8_owned_validated(
    ...
    ) -> Result<(CudaExecutionStats, super::CudaJpegDecodeStageTimings), CudaError> {
        ...
        let mut stage_profiler = CudaJpegDecodeStageProfiler::new(collect_stage_timings);
        ...
        stage_profiler.finish_resource_upload();
        ...
        self.context()
            .record_status_device_to_host_copy(status_buffer.byte_len());
        stage_profiler.finish_status_readback();
        ...
        let stage_timings =
            stage_profiler.finish(workspace_plan.map_or(0, |workspace| workspace.byte_len))?;
        Ok((
            CudaExecutionStats::new(kernel_dispatches, 0, kernel_dispatches, false),
            stage_timings,
        ))
    }
    ...
    fn launch_jpeg_decode_rgb8(
        ...
        self.launch_kernel(kernel, geometry, &mut kernel_params)
    }
