...
use crate::{error::CudaError, memory::CudaDeviceBuffer, JpegCudaEngine};
...
pub(super) fn validate_jpeg_context_matches(
    ...
    if matches_context.into_iter().all(|matches| matches) {
        ...
        Err(CudaError::InvalidArgument {
            message: JPEG_CONTEXT_MISMATCH.to_string(),
        })
    }
}
...
pub(super) fn validate_jpeg_buffer_context<'a>(
    ...
    validate_jpeg_context_matches(
        buffers
            .into_iter()
            .map(|buffer| engine.context.is_same_context(&buffer.context())),
    )
}
