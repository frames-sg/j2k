...
use crate::CudaJpegProfiledOutput;
use crate::{
    error::CudaError,
...
impl JpegCudaEngine<'_> {
    ...
    pub fn decode_jpeg_rgb8_owned_with_external_live(
        ...
        #[cfg(feature = "cuda-oxide-jpeg-decode")]
        {
            ...
            let (execution, _) =
                self.decode_jpeg_rgb8_owned_validated(plan, &output, validated, statuses, false)?;
            Ok(CudaKernelOutput::new(output, execution))
        }
    }

    /// Profile the existing route while charging host owners retained by the caller.
    #[doc(hidden)]
    pub fn profile_decode_jpeg_rgb8_owned_with_external_live(
        &self,
        plan: &CudaJpegRgb8DecodePlan<'_>,
        external_live_bytes: usize,
        ...
        #[cfg(feature = "cuda-oxide-jpeg-decode")]
        {
            ...
            let output = self.allocate(validated.output_len)?;
            let (execution, stage_timings) =
                self.decode_jpeg_rgb8_owned_validated(plan, &output, validated, statuses, true)?;
            Ok(CudaJpegProfiledOutput {
                output: CudaKernelOutput::new(output, execution),
                stage_timings,
            })
        }
    }
...
fn allocate_decode_statuses_with_cap(
    ...
    Ok(host_budget.try_vec_filled(count, CudaJpegDecodeStatus::default())?)
}
