...
use crate::{
    error::CudaError, execution::CudaExecutionStats, memory::CudaPinnedUploadOperationGuard,
    JpegCudaEngine,
};
...
impl JpegCudaEngine<'_> {
    ...
    pub fn diagnose_jpeg_420_entropy_self_sync_with_pinned_upload_operation(
        ...
        pinned_upload.ensure_for_context(self.context)?;
        ...
        if subsequences == 0 {
            return Ok(CudaJpegChunkedEntropyReport {
                ...
                states: Vec::new(),
                overflows: Vec::new(),
                execution: CudaExecutionStats::new(0, 0, 0, false),
            });
        }
