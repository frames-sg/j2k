...
const JPEG_CHECKPOINT_THREADS_PER_BLOCK: u32 = 128;
const JPEG_PACKED_CHECKPOINT_MIN_COUNT: u32 = 128;

pub(super) fn jpeg_checkpoint_launch_geometry(checkpoint_count: u32) -> Option<CudaLaunchGeometry> {
    let (grid_x, block_x) = if checkpoint_count < JPEG_PACKED_CHECKPOINT_MIN_COUNT {
        (checkpoint_count, 1)
    } else {
        (
            checkpoint_count.div_ceil(JPEG_CHECKPOINT_THREADS_PER_BLOCK),
            JPEG_CHECKPOINT_THREADS_PER_BLOCK,
        )
    };
