...
use crate::{
    ...
    CudaJpegEntropyCheckpoint, JpegCudaEngine,
};
...
fn safe_jpeg_apis_reject_foreign_buffers_and_keep_empty_batch_noop() {
    ...
    let engine = JpegCudaEngine::new(&context);
    ...
    assert!(validate_jpeg_buffer_context(engine, [&local_buffer]).is_ok());

    let checkpoints = [CudaJpegEntropyCheckpoint::default()];
    assert_jpeg_context_mismatch(engine.decode_jpeg_rgb8_owned_into(
        &decode_plan(&checkpoints),
        &foreign_buffer,
        3,
    ));
    assert_jpeg_context_mismatch(engine.encode_jpeg_baseline_entropy(&encode_job(&foreign_buffer)));
    assert_jpeg_context_mismatch(engine.encode_jpeg_baseline_entropy_batch(&encode_batch_job(
        &foreign_buffer,
...
fn safe_jpeg_encode_apis_reject_invalid_ranges_before_kernel_launch() {
    ...
    assert_jpeg_invalid_argument(
        engine.encode_jpeg_baseline_entropy(&single),
        "beyond allocation",
    );
    ...
    assert_jpeg_invalid_argument(
        engine.encode_jpeg_baseline_entropy_batch(&batch),
