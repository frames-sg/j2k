...
#[cfg(feature = "cuda-runtime")]
use signinum_cuda_runtime::{CudaContext, CudaDeviceBuffer};
#[cfg(feature = "cuda-runtime")]
...
pub struct CudaSession {
    submissions: u64,
    #[cfg(feature = "cuda-runtime")]
    owned_fast420_packets: VecDeque<CachedOwnedFastPacket<JpegMetalFast420PacketV1>>,
    #[cfg(feature = "cuda-runtime")]
    owned_fast422_packets: VecDeque<CachedOwnedFastPacket<JpegMetalFast422PacketV1>>,
    #[cfg(feature = "cuda-runtime")]
    owned_fast444_packets: VecDeque<CachedOwnedFastPacket<JpegMetalFast444PacketV1>>,
    #[cfg(feature = "cuda-runtime")]
    context: Option<CudaContext>,
    #[cfg(feature = "cuda-runtime")]
    owned_output_pool: Arc<Mutex<Vec<CudaDeviceBuffer>>>,
}
...
impl CudaSession {
    ...
    /// Number of cached Signinum-owned CUDA fast JPEG packets.
    ...
    pub fn take_owned_cuda_output_buffer(
    ...
    ) -> Result<CudaDeviceBuffer, Error> {
        {
            let mut pool = self
                .owned_output_pool
                .lock()
                .map_err(|error| Error::CudaRuntime {
                    message: format!("CUDA owned output pool lock poisoned: {error}"),
                })?;
            if let Some(index) = pool.iter().position(|buffer| buffer.byte_len() >= byte_len) {
                return Ok(pool.swap_remove(index));
            }
        }

        self.cuda_context()?.allocate(byte_len).map_err(cuda_error)
    }
    ...
    pub(crate) fn resolve_owned_fast420_packet(
        &mut self,
        input: &[u8],
    ) -> Result<Arc<JpegMetalFast420PacketV1>, Error> {
        resolve_owned_packet(
            &mut self.owned_fast420_packets,
            input,
            build_metal_fast420_packet,
        )
    }
    ...
    pub(crate) fn resolve_owned_fast422_packet(
        &mut self,
        input: &[u8],
    ) -> Result<Arc<JpegMetalFast422PacketV1>, Error> {
        resolve_owned_packet(
            &mut self.owned_fast422_packets,
            input,
            build_metal_fast422_packet,
        )
    }
    ...
    pub(crate) fn resolve_owned_fast444_packet(
        &mut self,
        input: &[u8],
    ) -> Result<Arc<JpegMetalFast444PacketV1>, Error> {
        resolve_owned_packet(
            &mut self.owned_fast444_packets,
            input,
            build_metal_fast444_packet,
        )
    }
...
#[cfg(feature = "cuda-runtime")]
fn owned_packet_error(error: MetalFast420PacketError) -> Error {
    match error {
        MetalFast420PacketError::Decode(error) => Error::Decode(error),
...
fn resolve_owned_packet<T>(
    ...
    input: &[u8],
    build: fn(&[u8]) -> Result<T, MetalFast420PacketError>,
) -> Result<Arc<T>, Error> {
