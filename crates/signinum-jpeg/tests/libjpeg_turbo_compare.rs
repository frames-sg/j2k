...
fn turbojpeg_rgb_and_region_match_signinum_fixture() {
    let require_turbo = std::env::var_os("SIGNINUM_REQUIRE_LIBJPEG_TURBO").is_some();
    let turbo_available = libjpeg_turbo::is_available();
    assert!(
        !require_turbo || turbo_available,
        "SIGNINUM_REQUIRE_LIBJPEG_TURBO is set but libjpeg-turbo is unavailable"
    );
    if !turbo_available {
        return;
    }
    ...
    let (rgb, _) = dec.decode(PixelFormat::Rgb8).expect("signinum rgb");
