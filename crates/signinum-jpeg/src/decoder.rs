...
impl Decoder<'_> {
        ...
        S: RowSink<u8, Error = JpegError>,
    {
        ...
        curr_row.resize(width, 0);
        if rgb_row.len() < width.saturating_mul(3) {
            return Err(JpegError::OutputBufferTooSmall {
                required: width.saturating_mul(3),
        ...
        for y in 0..height as usize {
            for x in 0..width {
                ...
                let predictor = if restart > 0 && samples_since_restart == 0 {
                    ...
                    lossless_predictor_gray_rows(plan.predictor, curr_row, prev_row, x, y)
                };
                let diff = plan.dc_table.decode_fast_dc(&mut br)?;
                let sample = predictor + diff;
                if !(0..=255).contains(&sample) {
                ...
                curr_row[x] = sample as u8;
                samples_since_restart += 1;
            }
            for (pixel, &sample) in rgb_row[..width * 3]
                .chunks_exact_mut(3)
                .zip(curr_row.iter())
            {
                pixel.copy_from_slice(&[sample, sample, sample]);
            }
            sink.write_row(y as u32, &rgb_row[..width * 3])?;
        ...
        S: RowSink<u8, Error = JpegError>,
    {
        ...
        let (width, height) = plan.dimensions;
        let width = width as usize;
        let row_len = width.saturating_mul(3);
        ...
        for y in 0..height as usize {
            for x in 0..width {
                ...
                for component in &self.plan.components {
                    ...
                    curr_row[x * 3 + component.output_index] = sample as u8;
                }
                pixels_since_restart += 1;
            }
        ...
        S: RowSink<u8, Error = JpegError>,
    {
        ...
        let (width, height) = plan.dimensions;
        let width = width as usize;
        let row_len = width.saturating_mul(2);
        prev_row.resize(row_len, 0);
        curr_row.resize(row_len, 0);
        ...
        for y in 0..height as usize {
            for x in 0..width {
                ...
                let predictor = if restart > 0 && samples_since_restart == 0 {
                    ...
                    lossless_predictor_gray16_rows(plan.predictor, curr_row, prev_row, x, y)
                };
                let diff = plan.dc_table.decode_fast_dc(&mut br)?;
                let sample = predictor + diff;
    ...
    fn decode_lossless_color16_sampled_into(
        ...
        let mut c1 = vec![0u16; chroma_width * chroma_height];
        let mut c2 = vec![0u16; chroma_width * chroma_height];
...
fn lossless_predictor_gray_rows(
    predictor: u8,
    curr_row: &[u8],
    prev_row: &[u8],
    x: usize,
    ...
    if y == 0 {
        return i32::from(curr_row[x - 1]);
    }
    if x == 0 {
        return i32::from(prev_row[0]);
    }

    let ra = i32::from(curr_row[x - 1]);
    let rb = i32::from(prev_row[x]);
    let rc = i32::from(prev_row[x - 1]);
    match predictor {
        1 => ra,
        2 => rb,
        3 => rc,
...
fn lossless_predictor_rgb8_rows(
    predictor: u8,
    curr_row: &[u8],
    prev_row: &[u8],
    x: usize,
    ...
    if y == 0 {
        return i32::from(curr_row[(x - 1) * 3 + component]);
    }
    if x == 0 {
        return i32::from(prev_row[component]);
    }

    let ra = i32::from(curr_row[(x - 1) * 3 + component]);
    let rb = i32::from(prev_row[x * 3 + component]);
    let rc = i32::from(prev_row[(x - 1) * 3 + component]);
    match predictor {
        1 => ra,
        2 => rb,
        3 => rc,
...
fn lossless_predictor_gray16_rows(
    predictor: u8,
    curr_row: &[u8],
    prev_row: &[u8],
    x: usize,
    ...
    if y == 0 {
        return i32::from(read_gray16_sample(curr_row, (x - 1) * 2));
    }
    if x == 0 {
        return i32::from(read_gray16_sample(prev_row, 0));
    }

    let ra = i32::from(read_gray16_sample(curr_row, (x - 1) * 2));
    let rb = i32::from(read_gray16_sample(prev_row, x * 2));
    let rc = i32::from(read_gray16_sample(prev_row, (x - 1) * 2));
    match predictor {
        1 => ra,
        2 => rb,
        3 => rc,
...
fn lossless_predictor_value_u16(
    ...
    if y == 0 {
        return i32::from(read_gray16_sample(out, (x - 1) * 2));
    }
    if x == 0 {
        return i32::from(read_gray16_sample(out, (y - 1) * stride));
    }

    let row = y * stride;
    let prev_row = (y - 1) * stride;
    let ra = i32::from(read_gray16_sample(out, row + (x - 1) * 2));
    let rb = i32::from(read_gray16_sample(out, prev_row + x * 2));
    let rc = i32::from(read_gray16_sample(out, prev_row + (x - 1) * 2));
    match predictor {
        1 => ra,
        2 => rb,
        3 => rc,
