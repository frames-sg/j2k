...
fn main() {
    ...
    for input in &inputs {
        ...
        let signinum_ns = median_ns(iterations, || {
            signinum_decode_tile_batch_sequential(&input.bytes, TILE_BATCH);
        });
...
fn render_csv(rows: &[BreakdownRow]) -> String {
    let mut csv = String::from(
        "input,signinum_ns,turbo_ns,turbo_speedup,profile_total_ns,parse_plan_ns,mcu_decode_ns,rgb_emit_ns,finish_ns,total_blocks,dc_only_blocks,bottom_half_zero_blocks,general_blocks\n",
    );
    for row in rows {
        ...
        csv.push_str(&format!(
            ...
            row.turbo_ns
                .map_or_else(String::new, |turbo| ratio(row.signinum_ns, turbo)),
...
fn mean_turbo_speedup(rows: &[BreakdownRow]) -> Option<f64> {
    ...
    for row in rows {
        if let Some(turbo) = row.turbo_ns {
            total += row.signinum_ns as f64 / turbo as f64;
            count += 1;
        }
    }
