...
fn main() {
    ...
    if std::env::var_os("SIGNINUM_BENCH_INPUTS").is_some() {
...
fn run_input(input: &BenchInput, iterations: usize) -> Vec<ReportRow> {
    ...
    match (input.mode, input.input_class) {
            ...
            if should_compare_full_frame(input.mode, input.input_class) =>
        {
            ...
            rows.push(run_signinum_only_row(
                input,
                Operation::DecodeRowsRgb,
                iterations_for(input, Operation::DecodeRowsRgb, iterations),
                "comparator skipped for very large RGB input; report uses signinum decode_rows",
            ));
        }
        (DecodeMode::Rgb, CorpusInputClass::VeryLarge) => {
            rows.push(run_signinum_only_row(
...
fn run_compare_row(input: &BenchInput, operation: Operation, iterations: usize) -> ReportRow {
    let (signinum, jpeg_decoder, zune) = run_compare_measurements(operation, input, iterations);
    ReportRow {
...
fn run_signinum_only_row(
    ...
    ReportRow {
        ...
        input_class: input.input_class,
        operation,
        signinum: run_measurement(Library::Signinum, operation, input, iterations),
...
fn is_supported(library: Library, operation: Operation, input: &BenchInput) -> bool {
    matches!(
        ...
        (Library::Signinum, Operation::Inspect, _, _)
...
fn run_compare_measurements(
    ...
    let mut signinum = MeasurementState::new(
        Library::Signinum,
        is_supported(Library::Signinum, operation, input),
        iterations,
    );
    ...
    let mut states = [&mut signinum, &mut jpeg_decoder, &mut zune];
    ...
    (signinum.finish(), jpeg_decoder.finish(), zune.finish())
}
...
fn run_operation(library: Library, operation: Operation, input: &BenchInput) {
    match (library, operation) {
        (Library::Signinum, Operation::Inspect) => signinum_inspect(&input.bytes),
        ...
        (Library::Signinum, Operation::DecodeRgb) => signinum_decode(&input.bytes, DecodeMode::Rgb),
        ...
        (Library::Signinum, Operation::DecodeGray) => {
            signinum_decode(&input.bytes, DecodeMode::Gray)
        }
        ...
        (Library::Signinum, Operation::DecodeRowsRgb) => signinum_decode_rows(&input.bytes),
        (Library::Signinum, Operation::WsiRegionRgb) => {
            signinum_decode_region(&input.bytes, ROI_SIDE)
        }
        ...
        (Library::Signinum, Operation::WsiScaledRgbQ4) => {
            signinum_decode_scaled(&input.bytes, Downscale::Quarter);
        }
        ...
        (Library::Signinum, Operation::WsiScaledRgbQ8) => {
            signinum_decode_scaled(&input.bytes, Downscale::Eighth);
        }
        ...
        (Library::Signinum, Operation::WsiRegionScaledRgbQ4) => {
            signinum_decode_region_scaled(&input.bytes, ROI_SIDE, Downscale::Quarter);
        }
        ...
        (Library::Signinum, Operation::WsiRegionScaledRgbQ8) => {
            signinum_decode_region_scaled(&input.bytes, ROI_SIDE, Downscale::Eighth);
        }
        ...
        (Library::Signinum, Operation::WsiTileBatchScaledRgbQ4) => {
            signinum_decode_tile_batch_scaled(&input.bytes, TILE_BATCH, Downscale::Quarter);
        }
        ...
        (Library::Signinum, Operation::WsiTileBatchRegionScaledRgbQ4) => {
...
fn render_csv(rows: &[ReportRow]) -> String {
    let mut csv = String::from(
        "input,mode,class,operation,signinum_ns,jpeg_decoder_ns,zune_ns,signinum_error,jpeg_decoder_error,zune_error,fastest\n",
    );
    for row in rows {
        ...
        csv.push_str(&format!(
            ...
            render_ns(&row.signinum),
            render_ns(&row.jpeg_decoder),
            render_ns(&row.zune),
            escape_csv(row.signinum.error.as_deref().unwrap_or("")),
...
fn render_markdown(rows: &[ReportRow], iterations: usize) -> String {
    ...
    md.push_str("# Signinum JPEG corpus report\n\n");
    md.push_str(&format!(
        "- inputs: {}\n",
        rows.iter()
    ...
    for row in rows {
        ...
        if fastest == Some("signinum") || fastest.is_none() {
            continue;
        }
        any_slower = true;
        md.push_str(&format!(
            ...
            row.input_name,
            row.operation.as_str(),
            format_measurement(&row.signinum),
    ...
    for row in rows {
        ...
        md.push_str(&format!(
            ...
            row.input_name,
            row.operation.as_str(),
            row.signinum.error.as_deref().unwrap_or("ok"),
...
impl Summary {
    fn accumulate(&mut self, row: &ReportRow) {
        if fastest_label(row) == Some("signinum") {
            self.signinum_fastest += 1;
        }
        match compare_measurements(&row.signinum, &row.jpeg_decoder) {
            Some(Ordering::Less) => self.vs_jpeg_wins += 1,
            Some(Ordering::Greater) => self.vs_jpeg_losses += 1,
            _ => {}
        }
        match compare_measurements(&row.signinum, &row.zune) {
            Some(Ordering::Less) => self.vs_zune_wins += 1,
            Some(Ordering::Greater) => self.vs_zune_losses += 1,
            _ => {}
        }
        if row.signinum.error.is_some()
...
fn fastest_label(row: &ReportRow) -> Option<&'static str> {
    ...
    for measurement in [&row.signinum, &row.jpeg_decoder, &row.zune] {
