...
mod tests {
    ...
    fn profile_row_uses_compact_key_value_format() {
        let row = format_profile_row(
            ...
            "gpu_route",
            "metal",
            &[("request", "Metal"), ("decision", "MetalKernel")],
        );
        assert_eq!(
            row,
            "signinum_profile codec=j2k op=gpu_route path=metal request=Metal decision=MetalKernel"
        );
    }
    ...
    fn profile_summary_counts_route_decisions() {
        ...
        assert_eq!(
            summary.format_rows(),
            vec![
                "signinum_profile_summary codec=j2k op=gpu_route path=metal request=Metal fmt=Rgb8 decision=metal_kernel reason=none count=2"
            ]
        );
    }
}
