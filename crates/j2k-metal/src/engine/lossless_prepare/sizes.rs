...
#[cfg(target_os = "macos")]
#[derive(Clone, Copy)]
pub(in crate::engine) struct J2kLosslessPrepareSizes {
    pub(in crate::engine) plane_len: usize,
    pub(in crate::engine) plane_bytes: usize,
    pub(in crate::engine) coefficient_bytes: usize,
}

#[cfg(target_os = "macos")]
pub(in crate::engine) fn lossless_prepare_sizes(
    ...
    if job.component_count != 1 && job.component_count != 3 {
        return Err(Error::capability_rejected(
            j2k_core::CapabilityRejection::unsupported_operation(
                "J2K Metal resident encode supports grayscale or RGB input",
            ),
        ));
    }
    if job.bytes_per_sample != 1 && job.bytes_per_sample != 2 {
        return Err(Error::capability_rejected(
            j2k_core::CapabilityRejection::unsupported_operation(
                "J2K Metal resident encode supports 8-bit or 16-bit samples",
            ),
        ));
    }
