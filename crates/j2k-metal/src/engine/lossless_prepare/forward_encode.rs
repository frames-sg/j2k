...
use super::{
    ...
    J2kQuantizeSubbandJob, J2kQuantizeSubbandParams, J2K_MCT_STATUS_OK,
};
    ...
    if plane1.len() != len || plane2.len() != len {
        return Err(Error::capability_rejected(
            j2k_core::CapabilityRejection::geometry_mismatch(
                "J2K Metal forward ICT plane lengths must match",
            ),
        ));
    }
    let len_u32 = u32::try_from(len).map_err(|_| {
        Error::capability_rejected(j2k_core::CapabilityRejection::resource_limit(
            "J2K Metal forward ICT plane length exceeds u32",
        ))
    })?;
...
#[cfg(target_os = "macos")]
pub(in crate::engine) fn validate_encode_quantize_subband_job(
    job: J2kQuantizeSubbandJob<'_>,
) -> Result<(), Error> {
    if job.step_exponent > 31 {
        return Err(Error::capability_rejected(
            j2k_core::CapabilityRejection::unsupported_operation(
                "J2K Metal encode quantize_subband supports step exponents <= 31",
            ),
        ));
    }
    ...
    if job.range_bits == 0 || job.range_bits > 31 {
        return Err(Error::capability_rejected(
            j2k_core::CapabilityRejection::geometry_mismatch(
                "J2K Metal encode quantize_subband supports range bits 1-31",
            ),
        ));
    }
    Ok(())
...
pub(crate) fn encode_quantize_subband(job: J2kQuantizeSubbandJob<'_>) -> Result<Vec<i32>, Error> {
    ...
    let len_u32 = u32::try_from(len).map_err(|_| {
        Error::capability_rejected(j2k_core::CapabilityRejection::resource_limit(
            "J2K Metal encode quantize_subband coefficient count exceeds u32",
        ))
    })?;
