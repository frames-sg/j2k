...
use super::{
    ...
    label_compute_encoder, new_compute_command_encoder, new_resident_encode_command_buffer, Buffer,
    CommandBuffer, CommandBufferRef, Error, J2kBatchedPacketPayloadCopyDispatch,
    ...
    J2kPacketPayloadCopyParams, MetalRuntime, PACKET_PAYLOAD_COPY_BYTES_PER_STRIPE,
    PACKET_PAYLOAD_COPY_STRIPES_PER_JOB,
};

pub(in crate::engine) struct ForwardDwt53SplitProfile<T> {
    pub(in crate::engine) active: T,
    pub(in crate::engine) vertical_command_buffers: Vec<CommandBuffer>,
    pub(in crate::engine) horizontal_command_buffers: Vec<CommandBuffer>,
}

#[cfg(target_os = "macos")]
pub(in crate::engine) fn dispatch_batched_packet_payload_copy(
    ...
    encoder.set_bytes::<J2kPacketPayloadCopyParams>(5, &params);
    encoder.dispatchThreads_threadsPerThreadgroup(
        j2k_metal_support::mtl_size(
            dispatch.max_payload_copy_jobs_per_tile,
            dispatch.tile_count,
            u64::from(PACKET_PAYLOAD_COPY_STRIPES_PER_JOB),
        ),
        j2k_metal_support::mtl_size(
            runtime
...
#[cfg(target_os = "macos")]
pub(in crate::engine) fn dispatch_lossless_deinterleave(
    runtime: &MetalRuntime,
    command_buffer: &CommandBufferRef,
...
#[cfg(target_os = "macos")]
pub(in crate::engine) fn dispatch_lossless_deinterleave_rct_rgb8(
    runtime: &MetalRuntime,
    command_buffer: &CommandBufferRef,
    job: J2kLosslessDevicePrepareJob<'_>,
    plane0: &Buffer,
...
#[cfg(target_os = "macos")]
pub(in crate::engine) fn lossless_deinterleave_rct_rgb8_supported(
    job: J2kLosslessDevicePrepareJob<'_>,
) -> bool {
...
#[cfg(target_os = "macos")]
pub(in crate::engine) fn dispatch_forward_rct_on_buffers(
    runtime: &MetalRuntime,
    command_buffer: &CommandBufferRef,
    plane0: &Buffer,
    plane1: &Buffer,
...
#[cfg(target_os = "macos")]
pub(in crate::engine) fn dispatch_forward_dwt53_on_buffers(
    runtime: &MetalRuntime,
    command_buffer: &CommandBufferRef,
    input: &Buffer,
    scratch: &Buffer,
...
#[cfg(target_os = "macos")]
#[derive(Clone, Copy)]
pub(in crate::engine) struct ForwardDwt53ComponentsDispatch<'a> {
    pub(in crate::engine) runtime: &'a MetalRuntime,
    pub(in crate::engine) command_buffer: &'a CommandBufferRef,
    pub(in crate::engine) plane_buffers: &'a [Buffer],
    pub(in crate::engine) scratch_buffers: &'a [Buffer],
    pub(in crate::engine) width: u32,
    pub(in crate::engine) height: u32,
    pub(in crate::engine) num_levels: u8,
    pub(in crate::engine) component_count: usize,
}

#[cfg(target_os = "macos")]
pub(in crate::engine) fn dispatch_forward_dwt53_components_on_buffers(
...
#[cfg(target_os = "macos")]
pub(in crate::engine) fn dispatch_forward_dwt53_on_buffers_split_profile(
    runtime: &MetalRuntime,
    input: &Buffer,
    scratch: &Buffer,
    width: u32,
...
#[cfg(target_os = "macos")]
pub(in crate::engine) fn dispatch_forward_dwt53_components_split_profile(
    runtime: &MetalRuntime,
...
#[cfg(target_os = "macos")]
pub(in crate::engine) fn dispatch_lossless_extract_coefficients(
    runtime: &MetalRuntime,
    command_buffer: &CommandBufferRef,
    planes: &[Buffer],
