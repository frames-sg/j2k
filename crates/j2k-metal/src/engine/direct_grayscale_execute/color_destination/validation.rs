...
pub(super) fn validate_color_group(
    ...
    if !matches!(
        ...
        return Err(Error::capability_rejected(
            j2k_core::CapabilityRejection::unsupported_format(
                "J2K Metal exact color destination supports native RGB/RGBA integers only",
            ),
        ));
    }
    if !matches!(layout, BatchLayout::Nchw | BatchLayout::Nhwc) {
        return Err(Error::capability_rejected(
            j2k_core::CapabilityRejection::unsupported_format(
                "J2K Metal exact RGB destination received an unknown batch layout",
            ),
        ));
    }
    if !color_bit_depths_supported(first, fmt) {
        return Err(Error::capability_rejected(
            j2k_core::CapabilityRejection::unsupported_bit_depth(
    ...
    {
        return Err(Error::capability_rejected(j2k_core::CapabilityRejection::unsupported_operation("J2K Metal exact color destination contains a code-block plan unsupported by the Metal runtime")));
    }
    destination
        ...
        || destination_layout.image_stride_bytes() != tight_image_bytes
    {
        return Err(Error::capability_rejected(
            j2k_core::CapabilityRejection::geometry_mismatch(
                "J2K Metal exact RGB destination must be one dense contiguous group",
            ),
        ));
    }
    Ok(())
...
fn color_bit_depths_supported(plan: &PreparedDirectColorPlan, fmt: PixelFormat) -> bool {
    let bit_depths = plan.bit_depths.iter().copied().chain(plan.alpha_bit_depth);
    match fmt {
        PixelFormat::Rgb8 | PixelFormat::Rgba8 => {
            !plan.signed && bit_depths.clone().all(|depth| (1..=8).contains(&depth))
        }
        PixelFormat::Rgb16 | PixelFormat::Rgba16 => {
            !plan.signed && bit_depths.clone().all(|depth| (9..=16).contains(&depth))
        }
        PixelFormat::RgbI16 | PixelFormat::RgbaI16 => {
            plan.signed && bit_depths.clone().all(|depth| (1..=16).contains(&depth))
        }
        _ => false,
    }
}
