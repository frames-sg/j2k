...
pub(in crate::engine) use self::execution::encode_prepared_direct_component_plane_in_encoder;

#[cfg(target_os = "macos")]
pub(in crate::engine) fn checked_coefficient_len(
    width: u32,
    height: u32,
    message: &str,
) -> Result<usize, Error> {
...
#[cfg(target_os = "macos")]
pub(in crate::engine) fn upload_cpu_decoded_coefficients(
...
#[cfg(target_os = "macos")]
pub(in crate::engine) struct DirectComponentPlaneRequest<'a> {
    pub(in crate::engine) runtime: &'a MetalRuntime,
    pub(in crate::engine) command_buffer: &'a CommandBufferRef,
    pub(in crate::engine) plan: &'a PreparedDirectGrayscalePlan,
    pub(in crate::engine) tier1_mode: DirectTier1Mode,
    pub(in crate::engine) stage_timings: &'a mut DirectHybridStageTimings,
    pub(in crate::engine) retained_buffers: &'a mut Vec<Buffer>,
    pub(in crate::engine) status_checks: &'a mut Vec<DirectStatusCheck>,
    pub(in crate::engine) scratch_buffers: &'a mut Vec<DirectScratchBuffer>,
}

#[cfg(target_os = "macos")]
pub(in crate::engine) fn encode_prepared_direct_component_plane_in_command_buffer(
