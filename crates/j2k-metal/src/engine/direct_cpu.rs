...
fn decode_prepared_classic_jobs_on_cpu_with_scratch_impl<const PROFILE: bool>(
    ...
    for job in jobs {
        ...
        if let Some(started) = prep_started {
            let counters = profile_counters.ok_or_else(|| Error::MetalStateInvariant {
                state: "J2K MetalDirect profiled classic CPU decode",
                reason: "profile counters are missing",
            })?;
            counters.record_classic_segment_prep(started);
        }
        if PROFILE {
            ...
            counters.record_classic_block_decode(decode_started, &profile);
        } else {
            let decode = if job.irreversible_midpoint != 0 {
                decode_j2k_code_block_scalar_with_workspace_midpoint
            } else {
    ...
    for job in jobs {
        ...
        if PROFILE {
            ...
            decode(decode_job, output_window, workspace, &mut profile)
                .map_err(native_decode_error)?;
            let counters = profile_counters.ok_or_else(|| Error::MetalStateInvariant {
                state: "J2K MetalDirect profiled HT CPU decode",
                reason: "profile counters are missing",
            })?;
            counters.record_ht_block_decode(decode_started, &profile);
        } else {
            let decode = if job.irreversible_midpoint != 0 {
                decode_ht_code_block_scalar_with_workspace_midpoint
            } else {
