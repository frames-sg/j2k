...
fn second_referenced_prepared_submission_uploads_no_immutable_arenas() {
    ...
    let runtime = crate::engine::MetalRuntime::new().expect("isolated Metal runtime");
    ...
    let prepared = crate::engine::prepare_referenced_htj2k_grayscale_plan(&referenced, &bytes)
    ...
    for submission in 0..2 {
        ...
        let output = crate::engine::new_shared_buffer(
            &runtime.device,
            group.total_coefficients * size_of::<f32>(),
        )
        .expect("referenced prepared output");
        let command_buffer = crate::engine::new_command_buffer(&runtime.queue)
            .expect("referenced prepared command buffer");
        let encoder = crate::engine::new_compute_command_encoder(&command_buffer)
        ...
        encoder.endEncoding();
        crate::engine::commit_and_wait_metal(&command_buffer)
            .expect("complete referenced prepared execution");
        crate::engine::validate_direct_status(&runtime, status)
