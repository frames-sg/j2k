...
#[cfg(target_os = "macos")]
pub(in crate::engine::direct_prepare) struct ReferencedClassicPayloadCursor<'a> {
    input: &'a [u8],
    payloads: &'a [J2kClassicCodeBlockPayload],
    ranges: &'a [J2kCodestreamRange],
    pub(in crate::engine::direct_prepare) next_payload: usize,
    next_range: usize,
}

#[cfg(target_os = "macos")]
impl<'a> ReferencedClassicPayloadCursor<'a> {
    pub(in crate::engine::direct_prepare) fn new(
        input: &'a [u8],
        payloads: &'a [J2kClassicCodeBlockPayload],
        ranges: &'a [J2kCodestreamRange],
    ) -> Self {
    ...
    pub(in crate::engine::direct_prepare) fn ensure_exhausted(&self) -> Result<(), Error> {
...
#[cfg(target_os = "macos")]
pub(in crate::engine::direct_prepare) fn prepare_referenced_classic_sub_band(
