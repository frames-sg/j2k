    ...
    let first = referenced
        ...
        .ok_or(Error::capability_rejected(
            j2k_core::CapabilityRejection::contract_violation(
                "J2K Metal grayscale prepared path received a color HTJ2K plan",
            ),
        ))?;
    ...
    for tile in referenced.tiles() {
        let geometry = tile.grayscale_geometry().ok_or(Error::capability_rejected(
            j2k_core::CapabilityRejection::geometry_mismatch(
                "J2K Metal grayscale prepared path received a color HTJ2K tile",
            ),
        ))?;
...
pub(crate) fn prepare_referenced_classic_grayscale_plan(
    ...
    let first = referenced
        ...
        .ok_or(Error::capability_rejected(
            j2k_core::CapabilityRejection::contract_violation(
                "J2K Metal grayscale prepared path received a color classic J2K plan",
            ),
        ))?;
    let step_count = crate::batch_allocation::checked_count_sum(
        referenced.tiles().iter().map(|tile| {
    ...
    for tile in referenced.tiles() {
        let geometry = tile.grayscale_geometry().ok_or(Error::capability_rejected(
            j2k_core::CapabilityRejection::geometry_mismatch(
                "J2K Metal grayscale prepared path received a color classic J2K tile",
            ),
        ))?;
        validate_referenced_component_metadata(first, geometry)?;
        let expected_end = validate_payload_record_span(
...
#[cfg(target_os = "macos")]
pub(in crate::engine) fn prepare_direct_grayscale_plan_for_cpu_upload(
    plan: &J2kDirectGrayscalePlan,
) -> Result<PreparedDirectGrayscalePlan, Error> {
