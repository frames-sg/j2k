        ...
        match step {
            ...
            J2kDirectGrayscaleStep::Idwt(idwt) => {
                ...
                match idwt.transform {
                    slidecodec_j2k_native::J2kWaveletTransform::Reversible53 => {
                        compute::decode_reversible53_single_decomposition_idwt(job, &mut output)?;
                    }
                    slidecodec_j2k_native::J2kWaveletTransform::Irreversible97 => {
            ...
            J2kDirectGrayscaleStep::Store(store) => {
                ...
                let job = slidecodec_j2k_native::J2kStoreComponentJob {
                    input,
...
fn decode_classic_sub_band(plan: &J2kOwnedSubBandPlan, output: &mut [f32]) -> Result<(), Error> {
    ...
    compute::decode_classic_cleanup_sub_band(
        slidecodec_j2k_native::J2kSubBandDecodeJob {
            width: plan.width,
            height: plan.height,
            jobs: &jobs,
        },
...
fn decode_ht_sub_band(plan: &HtOwnedSubBandPlan, output: &mut [f32]) -> Result<(), Error> {
    ...
    compute::decode_ht_cleanup_sub_band(
        slidecodec_j2k_native::HtSubBandDecodeJob {
            width: plan.width,
            height: plan.height,
            jobs: &jobs,
        },
...
mod tests {
    ...
    fn ht_plan() -> slidecodec_j2k_native::J2kDirectGrayscalePlan {
        let pixels: Vec<u8> = (0..16).collect();
        let options = EncodeOptions {
            reversible: true,
            num_decomposition_levels: 1,
        ...
        for step in &plan.steps {
            match step {
                ...
                J2kDirectGrayscaleStep::Idwt(idwt) => {
                    ...
                    let mut output =
                        vec![0.0f32; idwt.rect.width() as usize * idwt.rect.height() as usize];
                    let job = slidecodec_j2k_native::J2kSingleDecompositionIdwtJob {
                        rect: idwt.rect,
                        transform: idwt.transform,
                        ll: slidecodec_j2k_native::J2kIdwtBand {
                            rect: idwt.ll,
                            coefficients: ll,
                        },
                        hl: slidecodec_j2k_native::J2kIdwtBand {
                            rect: idwt.hl,
                            coefficients: hl,
                        },
                        lh: slidecodec_j2k_native::J2kIdwtBand {
                            rect: idwt.lh,
                            coefficients: lh,
                        },
                        hh: slidecodec_j2k_native::J2kIdwtBand {
                            rect: idwt.hh,
                            coefficients: hh,
                        },
                    };
    ...
    fn execute_to_first_idwt_inputs(
        ...
        slidecodec_j2k_native::J2kDirectIdwtStep,
        Vec<f32>,
        Vec<f32>,
        Vec<f32>,
        Vec<f32>,
    ...
    struct CaptureIdwtJob {
        rect: Option<slidecodec_j2k_native::J2kRect>,
        ll_rect: Option<slidecodec_j2k_native::J2kRect>,
        hl_rect: Option<slidecodec_j2k_native::J2kRect>,
        lh_rect: Option<slidecodec_j2k_native::J2kRect>,
        hh_rect: Option<slidecodec_j2k_native::J2kRect>,
        ll: Vec<f32>,
        hl: Vec<f32>,
        lh: Vec<f32>,
        hh: Vec<f32>,
    ...
    impl HtCodeBlockDecoder for CaptureIdwtJob {
        fn decode_code_block(
            &mut self,
            job: slidecodec_j2k_native::HtCodeBlockDecodeJob<'_>,
            output: &mut [f32],
        ) -> slidecodec_j2k_native::Result<()> {
            slidecodec_j2k_native::decode_ht_code_block_scalar(job, output)
        }
