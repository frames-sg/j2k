...
fn discover_grok_decompress_bin() -> Option<PathBuf> {
    std::env::var_os("SLIDECODEC_GROK_BIN")
        .map(PathBuf::from)
...
fn discover_grok_compress_bin() -> Option<PathBuf> {
    if let Some(path) = std::env::var_os("SLIDECODEC_GROK_COMPRESS_BIN")
        .map(PathBuf::from)
        .filter(|path| path.exists())
    {
        return Some(path);
...
fn temp_dir() -> &'static Path {
    ...
    DIR.get_or_init(|| {
        let dir =
            std::env::temp_dir().join(format!("slidecodec-j2k-grok-parity-{}", std::process::id()));
