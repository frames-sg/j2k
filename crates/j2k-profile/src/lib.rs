...
pub const GPU_ROUTE_PROFILE_ENV: &str = "J2K_GPU_ROUTE_PROFILE";
...
fn format_profile_prefix(codec: &str, op: &str, path: &str) -> String {
    ...
    write!(row, "j2k_profile codec={codec} op={op} path={path}").expect("writing to String failed");
    row
}
...
fn format_profile_summary_prefix(codec: &str, op: &str, path: &str) -> String {
    let mut row = String::new();
    write!(row, "j2k_profile_summary codec={codec} op={op} path={path}")
