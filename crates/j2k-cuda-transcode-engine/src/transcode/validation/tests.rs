...
use crate::{
    ...
    CudaHtj2k97QuantizeParams, CudaTranscodeEngine,
};
...
fn every_public_transcode_pool_api_rejects_a_foreign_pool_first() {
    ...
    let engine = CudaTranscodeEngine::new(&context);
    assert!(validate_transcode_pool_context(&engine, &local_pool).is_ok());

    assert_transcode_pool_context_mismatch(engine.j2k_transcode_dwt97_batch_with_pool(
    ...
    assert_transcode_pool_context_mismatch(
        engine.j2k_transcode_htj2k97_codeblock_batch_with_pool(f32_request(&foreign_pool)),
    );
    assert_transcode_pool_context_mismatch(
        engine.j2k_transcode_htj2k97_codeblock_batch_resident_with_pool(f32_request(&foreign_pool)),
    );
    assert_transcode_pool_context_mismatch(
        engine.j2k_transcode_htj2k97_codeblock_i16_batch_resident_with_pool(
