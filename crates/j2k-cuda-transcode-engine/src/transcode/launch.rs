...
use super::{
    types::{Dwt97ColumnLiftBatchLaunch, Dwt97QuantizeCodeblocksLaunch},
...
use crate::{
    ...
    kernels::{
        self, copy_u8_launch_geometry, transcode_dwt53_launch_geometry, with_grid_y, with_grid_z,
        CudaKernel,
    },
...
impl CudaTranscodeEngine<'_> {
    ...
    pub(super) fn launch_transcode_reversible53_vertical(
        ...
        let geometry = transcode_dwt53_launch_geometry(grid_w, grid_h)
    ...
    pub(super) fn transcode_kernel_function(
        kernel: CudaKernel,
    ) -> Result<j2k_cuda_runtime::CudaKernelSpec, CudaError> {
        kernel.spec()
    }
    ...
    pub(super) fn launch_transcode_dwt97_idct_batch_kernel(
        ...
        let grid_h =
            u32::try_from(dims.height).map_err(|_| CudaError::LengthTooLarge { len: 0 })?;
        let base = transcode_dwt53_launch_geometry(grid_w, grid_h)
            .ok_or(CudaError::LengthTooLarge { len: 0 })?;
    ...
    pub(super) fn launch_transcode_dwt97_row_lift_batch_coop(
        ...
        let function = Self::transcode_kernel_function(CudaKernel::TranscodeDwt97RowLiftBatchCoop)?;
        let mut spatial_ptr = spatial.device_ptr();
        let mut width = dims.width;
        let mut height = dims.height;
        let mut low_width = dims.low_width;
