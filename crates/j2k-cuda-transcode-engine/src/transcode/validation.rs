...
use crate::{
    ...
    context::ensure_context_ownership,
    error::CudaError,
...
pub(super) fn validate_transcode_pool_context(
    engine: &CudaTranscodeEngine<'_>,
    pool: &CudaBufferPool,
) -> Result<(), CudaError> {
    validate_transcode_pool_context_match(pool.is_owned_by(engine.context()))
}
...
pub(super) fn ensure_transcode_runtime_ptx_available() -> Result<(), CudaError> {
    if transcode_runtime_ptx_available() {
        ...
        Err(CudaError::InvalidArgument {
            message: "CUDA Oxide transcode PTX was not built; enable j2k-cuda-transcode-engine/cuda-oxide-transcode or a crate cuda-runtime feature that implies it, and use J2K_REQUIRE_CUDA_OXIDE_BUILD=1 on CUDA hosts"
                .to_string(),
        })
    }
}
