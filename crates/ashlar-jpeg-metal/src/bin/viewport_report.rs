...
use ashlar_jpeg::{ColorSpace, Decoder as CpuDecoder, ScratchPool};
use ashlar_jpeg_metal::viewport::{decode_viewport_to_surface, suggest_viewport_workload};
...
fn main() {
    ...
    if inputs.is_empty() {
        eprintln!(
            "viewport_report: no eligible JPEG inputs found; set ASHLAR_BENCH_INPUTS to extracted JPEG tiles or levels"
        );
        std::process::exit(2);
    }
...
fn load_bench_inputs() -> Vec<BenchInput> {
    ...
    let mut seen = inputs
        ...
        .collect::<Vec<_>>();
    for path in std::env::split_paths(&std::env::var_os("ASHLAR_BENCH_INPUTS").unwrap_or_default())
    {
