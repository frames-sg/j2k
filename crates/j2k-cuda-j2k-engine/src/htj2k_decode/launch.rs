...
impl crate::J2kCudaEngine<'_> {
    ...
    fn launch_htj2k_decode_kernel<const N: usize>(
        ...
        match mode {
            CudaLaunchMode::Sync => {
                // SAFETY: callers construct the exact ABI array for the selected
                // engine-owned entry point and retain every referenced buffer.
                unsafe { self.context.launch_compiled_kernel(spec, geometry, params) }
            }
            CudaLaunchMode::Async => {
                // SAFETY: the owning queued guard retains all referenced
                // buffers and establishes completion before reuse.
                unsafe {
                    self.context
