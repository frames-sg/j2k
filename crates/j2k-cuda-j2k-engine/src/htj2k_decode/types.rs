...
impl CudaHtj2kDecodeResources {
    /// Return the device-resident compressed payload shared by decode engines.
    #[doc(hidden)]
    pub fn payload_buffer(&self) -> Result<&CudaDeviceBuffer, CudaError> {
        self.payload.buffer()
    }

    /// Return the logical compressed-payload byte length.
    #[doc(hidden)]
    #[must_use]
