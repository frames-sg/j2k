...
impl CudaHtj2kDecodeResources {
    /// Return whether the payload and optional tables belong to `context`.
    #[doc(hidden)]
    pub fn is_owned_by(&self, context: &CudaContext) -> Result<bool, CudaError> {
...
fn validate_target_allocations_disjoint<T>(
    ...
    context
        .validate_disjoint_device_buffers(targets.iter().map(buffer))
