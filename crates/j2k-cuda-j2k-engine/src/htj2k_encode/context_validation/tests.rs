...
use crate::htj2k_encode::HTJ2K_UVLC_ENCODE_TABLE_BYTES;
use crate::{
    context::CudaContext,
    error::CudaError,
    htj2k_encode::{
...
fn resident_entry_points_reject_foreign_coefficients_before_empty_fast_paths() {
    ...
    assert_context_mismatch(
        crate::J2kCudaEngine::new(&fixture.context).encode_htj2k_codeblocks_resident(
    ...
    assert_context_mismatch(
        crate::J2kCudaEngine::new(&fixture.context).encode_htj2k_codeblock_regions_resident(
            &fixture.foreign_coefficients,
            0,
            &[],
            tables,
    ...
    assert_context_mismatch(
        crate::J2kCudaEngine::new(&fixture.context)
...
fn resource_and_pool_entry_points_reject_foreign_owners_when_empty() {
    ...
    assert_context_mismatch(
        crate::J2kCudaEngine::new(&fixture.context).encode_htj2k_codeblocks_with_resources(
            &[],
            &[],
...
fn matching_empty_batches_and_repeated_same_context_targets_remain_valid() {
    ...
    let compact = crate::J2kCudaEngine::new(&fixture.context)
        .encode_htj2k_codeblocks_multi_resident_compact_with_resources_and_pool(
            &[target, target],
            &fixture.local_resources,
            &fixture.local_pool,
