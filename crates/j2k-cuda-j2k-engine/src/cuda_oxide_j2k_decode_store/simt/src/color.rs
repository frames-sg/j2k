...
use crate::{
    ...
    sample::{round_ties_even_f32, sample_as_u8, sample_as_u16},
    transform::inverse_mct_sample,
};

#[inline(always)]
fn add_mct_level_shift(sample: f32, addend: f32, irreversible97: u32) -> f32 {
    let centered = if irreversible97 != 0 {
...
pub(crate) fn store_rgb8_mct_sample(
    ...
    store_u8(
        ...
        dst,
        sample_as_u8(
            add_mct_level_shift(out0, job.addend0, mct_job.irreversible97),
            job.bit_depth0,
        ),
    );
    store_u8(
        ...
        dst + 1,
        sample_as_u8(
            add_mct_level_shift(out1, job.addend1, mct_job.irreversible97),
            job.bit_depth1,
        ),
    );
    store_u8(
        ...
        dst + 2,
        sample_as_u8(
            add_mct_level_shift(out2, job.addend2, mct_job.irreversible97),
            job.bit_depth2,
        ),
    );
    if job.rgba != 0 {
