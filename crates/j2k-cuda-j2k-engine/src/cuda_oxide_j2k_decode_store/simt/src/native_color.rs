...
use crate::{
    ...
    sample::{
        round_ties_even_f32, sample_as_native_i16, sample_as_native_u8, sample_as_native_u16,
    },
    transform::inverse_mct_sample,
};

#[inline(always)]
fn add_native_level_shift(sample: f32, addend: f32, transform: u32) -> f32 {
    let centered = if transform == 2 {
        round_ties_even_f32(sample)
    } else {
        sample
    };
    centered + addend
...
pub(crate) fn exact_rgb_samples(
    ...
    (
        add_native_level_shift(out0, job.addend0, job.transform),
        add_native_level_shift(out1, job.addend1, job.transform),
        add_native_level_shift(out2, job.addend2, job.transform),
    )
}
