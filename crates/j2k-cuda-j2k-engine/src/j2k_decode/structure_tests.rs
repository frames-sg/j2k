...
const IDWT_SIMT: &str = include_str!("../cuda_oxide_j2k_idwt/simt/src/main.rs");
const MODULES: &[(&str, &str, usize)] = &[
    ...
    (
        "j2k_decode/idwt/launch_geometry.rs",
        include_str!("idwt/launch_geometry.rs"),
        75,
    ),
    (
    ...
    (
        "j2k_decode/idwt/normalization.rs",
        include_str!("idwt/normalization.rs"),
        125,
    ),
    (
        "j2k_decode/idwt/profiling.rs",
        include_str!("idwt/profiling.rs"),
        75,
    ),
    ...
    (
        "j2k_decode/idwt_launch/batch.rs",
        include_str!("idwt_launch/batch.rs"),
        125,
    ),
    (
        "j2k_decode/idwt_launch/profiling.rs",
        include_str!("idwt_launch/profiling.rs"),
        75,
    ),
    ...
    (
        "j2k_decode/store/tests/color_rounding.rs",
        include_str!("store/tests/color_rounding.rs"),
        75,
    ),
    (
        "j2k_decode/store/tests/color_rounding/display_mct.rs",
        include_str!("store/tests/color_rounding/display_mct.rs"),
        100,
    ),
    (
        "j2k_decode/store/tests/color_rounding/display_mct/rgb16.rs",
        include_str!("store/tests/color_rounding/display_mct/rgb16.rs"),
        100,
    ),
    (
        "j2k_decode/store/tests/color_rounding/rgba.rs",
        include_str!("store/tests/color_rounding/rgba.rs"),
        100,
    ),
    (
...
fn cuda_j2k_decode_uses_focused_real_modules() {
    ...
    assert!(
        IDWT.contains("mod launch_geometry;")
            && include_str!("idwt_launch.rs").contains("mod batch;"),
        "IDWT launch planning and batch orchestration must stay in focused modules"
    );
    assert!(
        IDWT_SIMT.contains("j2k_idwt_interleave_horizontal_multi"),
        "wide IDWT must retain the portable generic fallback"
    );
    assert!(
        IDWT.contains("mod preflight;")
            && IDWT.contains("mod launch_validation;")
            && IDWT.contains("mod normalization;")
            && IDWT.contains("mod sequence;"),
        "j2k_decode/idwt.rs must delegate preflight, launch validation, normalization, and sequence ownership"
    );
    assert!(
    ...
    assert!(
        STORE_TESTS.contains("mod color_native;")
            && STORE_TESTS.contains("mod color_rounding;")
