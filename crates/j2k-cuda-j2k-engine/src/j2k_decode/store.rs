...
impl crate::J2kCudaEngine<'_> {
    ...
    pub fn j2k_store_gray8_device(
        ...
        validate_store_buffer_context(self.context, [input])?;
        ...
        if output_bytes == 0 || pixels == 0 {
            let zero_fill_enqueued =
                zero_unwritten_store_output(self.context, &output, output_bytes, full_coverage)?;
        ...
        if zero_unwritten_store_output(self.context, &output, output_bytes, full_coverage)? {
            self.synchronize()?;
        }
    ...
    pub fn j2k_inverse_mct_device(
        ...
        validate_store_buffer_context(self.context, [plane0, plane1, plane2])?;
        ...
        Ok(CudaExecutionStats::new(1, 0, 1, false))
    }
