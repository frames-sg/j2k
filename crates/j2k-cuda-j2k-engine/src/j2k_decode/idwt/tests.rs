...
use crate::{
    CudaContext, CudaDeviceBuffer, CudaError, CudaJ2kIdwtJob, CudaJ2kIdwtNormalization,
    CudaJ2kIdwtTarget, CudaJ2kRect,
};

#[test]
fn normalization_metadata_preserves_legacy_boolean_jobs() {
    let mut irreversible = zero_job();
    irreversible.irreversible97 = u32::MAX;
    assert_eq!(
        super::normalized_idwt_job(irreversible, CudaJ2kIdwtNormalization::Standard).irreversible97,
        1
    );
    assert_eq!(
        super::normalized_idwt_job(irreversible, CudaJ2kIdwtNormalization::OpenJpegCodestream)
            .irreversible97,
        2
    );
    assert_eq!(
        super::normalized_idwt_job(zero_job(), CudaJ2kIdwtNormalization::OpenJpegCodestream)
            .irreversible97,
        0
    );
}
...
fn zero_work_batch_rejects_foreign_buffers_and_pool() {
    ...
    assert_invalid_argument_contains(
        crate::J2kCudaEngine::new(&context)
            .j2k_inverse_dwt_batch_device_with_pool(&foreign_target, &local_pool),
        "must belong to the launch context",
    );
    assert_invalid_argument_contains(
        crate::J2kCudaEngine::new(&context)
            .j2k_inverse_dwt_batch_device_with_pool(&[], &foreign_pool),
        "must belong to the launch context",
    );
}
...
fn zero_work_batch_rejects_output_input_aliasing() {
    ...
    assert_invalid_argument_contains(
        crate::J2kCudaEngine::new(&context).j2k_inverse_dwt_batch_device_with_pool(&target, &pool),
...
fn undersized_idwt_input_is_rejected_before_pool_allocation_or_launch() {
    ...
    assert_invalid_argument_contains(
        crate::J2kCudaEngine::new(&context).j2k_inverse_dwt_single_output_bytes(
            &undersized_ll,
            &band,
            &band,
            &band,
    ...
    assert_invalid_argument_contains(
        crate::J2kCudaEngine::new(&context).j2k_inverse_dwt_single_device_with_pool(
            &undersized_ll,
            &band,
            &band,
            &band,
    ...
    assert_invalid_argument_contains(
        crate::J2kCudaEngine::new(&context).j2k_inverse_dwt_batch_device_with_pool(&targets, &pool),
        "LL buffer is too small",
    );
}
