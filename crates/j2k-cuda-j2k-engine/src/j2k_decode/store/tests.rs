...
fn inverse_mct_rejects_aliasing_before_the_zero_length_fast_path() {
    ...
    assert_inverse_mct_overlap(crate::J2kCudaEngine::new(&context).j2k_inverse_mct_device(
        &plane,
        &plane,
        &plane,
...
fn safe_store_apis_reject_foreign_context_buffers_and_keep_empty_batches_valid() {
    ...
    assert_context_mismatch(
        crate::J2kCudaEngine::new(&context).j2k_store_gray8_device(&foreign_buffer, gray8_job),
    );
    assert_context_mismatch(
        crate::J2kCudaEngine::new(&context).j2k_store_gray16_device(&foreign_buffer, gray16_job),
    );
    assert_context_mismatch(crate::J2kCudaEngine::new(&context).j2k_inverse_mct_device(
        &foreign_buffer,
        &foreign_buffer,
        &foreign_buffer,
        inverse_mct_job,
    ));
    assert_context_mismatch(crate::J2kCudaEngine::new(&context).j2k_store_rgb8_device(
        &foreign_buffer,
        &foreign_buffer,
        &foreign_buffer,
        rgb8_job,
    ));
    assert_context_mismatch(crate::J2kCudaEngine::new(&context).j2k_store_rgb16_device(
        &foreign_buffer,
        &foreign_buffer,
        &foreign_buffer,
        rgb16_job,
    ...
    assert_context_mismatch(
        crate::J2kCudaEngine::new(&context).j2k_store_rgb8_mct_device(
            &foreign_buffer,
            &foreign_buffer,
            &foreign_buffer,
            rgb8_mct_job,
    ...
    assert_context_mismatch(
        crate::J2kCudaEngine::new(&context).j2k_store_rgb8_mct_batch_device(&[target]),
    );
    assert_context_mismatch(
        crate::J2kCudaEngine::new(&context).j2k_store_rgb8_mct_batch_contiguous_device(&[target]),
    );
    assert_context_mismatch(
        crate::J2kCudaEngine::new(&context).j2k_store_rgb16_mct_device(
            &foreign_buffer,
            &foreign_buffer,
            &foreign_buffer,
    ...
    let empty_batch = crate::J2kCudaEngine::new(&context)
...
fn partial_and_zero_copy_gray_outputs_are_zero_initialized_when_required() {
    ...
    let context = CudaContext::system_default().expect("CUDA context");
    let input = context.upload_f32(&[17.0]).expect("gray input");
    let partial = crate::J2kCudaEngine::new(&context)
        .j2k_store_gray8_device(
            &input,
            CudaJ2kStoreGray8Job {
                input_width: 1,
    ...
    let zero_copy = crate::J2kCudaEngine::new(&context)
        .j2k_store_gray8_device(
            &input,
            CudaJ2kStoreGray8Job {
                input_width: 1,
