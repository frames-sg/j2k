...
impl crate::J2kCudaEngine<'_> {
    ...
    > {
        ...
        let zero_fill_enqueued = plan.requires_zero_fill
            && zero_unwritten_store_output(self.context, &output, plan.total_bytes, false)?;
        let queued = self
            .enqueue_gray8_batch(targets, &plan, output.device_ptr())
            .and_then(|queued| {
                attach_zero_fill_completion(self.context, queued, zero_fill_enqueued)
            })
            .map_err(|error| retire_failed_zero_fill(self.context, error, zero_fill_enqueued))?;
    ...
    pub unsafe fn j2k_store_gray8_batch_into_external_device_enqueue(
        ...
        let base = external_destination_base(self.context, destination, &plan, 1)?;
    ...
    pub unsafe fn j2k_store_gray16_batch_into_external_device_enqueue(
        ...
        let base = external_destination_base(
            self.context,
            destination,
            &plan,
