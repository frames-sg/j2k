...
pub(in crate::j2k_decode) mod launch_geometry;
...
pub use normalization::CudaJ2kIdwtNormalization;
pub use profiling::CudaJ2kIdwtBatchStageProfile;
...
const CUDA_IDWT_CODESTREAM_97_MODE: u32 = 2;

fn normalized_idwt_job(
    mut job: CudaJ2kIdwtJob,
    normalization: CudaJ2kIdwtNormalization,
) -> CudaJ2kIdwtJob {
    if job.irreversible97 != 0 {
        job.irreversible97 = match normalization {
            CudaJ2kIdwtNormalization::Standard => 1,
            CudaJ2kIdwtNormalization::OpenJpegCodestream => CUDA_IDWT_CODESTREAM_97_MODE,
        };
    }
    job
}
...
impl crate::J2kCudaEngine<'_> {
    ...
    pub fn j2k_inverse_dwt_single_device_with_pool(
        ...
        self.j2k_inverse_dwt_single_device_with_pool_impl(J2kInverseDwtSinglePoolRequest {
            bands: [ll, hl, lh, hh],
            job,
            normalization: CudaJ2kIdwtNormalization::Standard,
    ...
    pub fn j2k_inverse_dwt_batch_device_with_pool(
        ...
        self.j2k_inverse_dwt_batch_device_with_pool_impl(
            targets,
    ...
    pub unsafe fn j2k_inverse_dwt_batch_device_enqueue_with_pool(
        ...
        validate_idwt_enqueue_context(self.context, targets, pool)?;
        let mut kernel_jobs = j2k_idwt_multi_kernel_jobs(targets)?;
        for kernel_job in &mut kernel_jobs {
            kernel_job.job =
                normalized_idwt_job(kernel_job.job, CudaJ2kIdwtNormalization::Standard);
        }
        ...
        let kernel_dispatches = match launch_result {
            Ok(kernel_dispatches) => kernel_dispatches,
            Err(error) => return pool_reuse_guard.synchronize_then_error(error),
        };

        Ok(CudaQueuedExecution {
            resources: queued_resources,
            execution: CudaExecutionStats::new(kernel_dispatches, 0, kernel_dispatches, false),
    ...
    fn j2k_inverse_dwt_single_device_impl(
        ...
        let job = normalized_idwt_job(job, CudaJ2kIdwtNormalization::Standard);
        let validated = validate_idwt_single_request(self.context, [ll, hl, lh, hh], job)?;
        ...
        Ok(CudaKernelOutput {
            buffer: output,
            execution: CudaExecutionStats::new(3, 0, 3, false),
        })
    }
    ...
    fn j2k_inverse_dwt_single_device_with_pool_impl(
        ...
        let job = normalized_idwt_job(request.job, request.normalization);
        let synchronize_each_launch = request.synchronize_each_launch;
        let pool = request.pool;
        if !pool.is_owned_by(self.context)
            || !idwt_inputs_belong_to_context(self.context, [ll, hl, lh, hh])
        {
