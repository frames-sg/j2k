...
use crate::{build_flags::ensure_j2k_ml_ptx_built, J2kCudaEngine};
use j2k_cuda_runtime::{
    cuda_kernel_param, CudaError, CudaExternalDeviceBufferViewMut, CudaKernelSpec,
    CudaLaunchGeometry,
};
...
impl J2kCudaEngine<'_> {
    ...
    pub fn j2k_ml_convert_into_external(
        ...
        let validated = validate_ml_launch(self, source_ptr, source_byte_len, destination, config)?;
        ensure_j2k_ml_ptx_built()?;
        let spec = CudaKernelSpec::new("j2k-ml", j2k_ml_ptx(), b"j2k_ml_convert\0")?;

        let mut destination_ptr = destination.device_ptr();
        let mut source_ptr = source_ptr;
        let mut sample_count = validated.sample_count as u64;
        let mut channels = config.channels;
        let mut source_sample = config.sample.flag();
        let mut output_kind = validated.output_kind;
        ...
        // SAFETY: all scalar ABI values remain live through this synchronous
        // call; source and destination pointers were validated for the bound
        // context, and the engine owns the matching PTX entry point.
        unsafe {
            self.context
                .launch_compiled_kernel(spec, validated.geometry, &mut params)
        }
    }
}

fn validate_ml_launch(
    engine: &J2kCudaEngine<'_>,
    source_ptr: u64,
    source_byte_len: usize,
    destination: &CudaExternalDeviceBufferViewMut<'_>,
    config: CudaJ2kMlKernelConfig,
) -> Result<ValidatedMlLaunch, CudaError> {
    if !engine.context.is_same_context(destination.context()) {
    ...
    engine.context.validate_device_pointer(source_ptr)?;

    let (output_width, output_kind) = match config.normalization {
        CudaJ2kMlNormalization::Integer => (config.sample.byte_width(), config.sample.flag()),
        CudaJ2kMlNormalization::Raw
        | CudaJ2kMlNormalization::Unit
        | CudaJ2kMlNormalization::MeanStd { .. } => (4, 4),
    };
    ...
    let grid_x = u32::try_from(sample_count.div_ceil(256))
        .map_err(|_| CudaError::LengthTooLarge { len: sample_count })?;
    let geometry = CudaLaunchGeometry::new((grid_x, 1, 1), (256, 1, 1))
        .ok_or(CudaError::LengthTooLarge { len: sample_count })?;
    Ok(ValidatedMlLaunch {
        sample_count,
        output_kind,
        geometry,
    })
...
#[cfg(all(test, feature = "cuda-oxide-j2k-ml"))]
mod tests {
    use super::{CudaJ2kMlKernelConfig, CudaJ2kMlLayout, CudaJ2kMlNormalization, CudaJ2kMlSample};
    use crate::J2kCudaEngine;
    use j2k_cuda_runtime::{CudaContext, CudaError, CudaExternalDeviceBufferViewMut};

    #[test]
    fn external_destination_checks_batch_offsets_before_launch_when_required() {
        ...
        let error = J2kCudaEngine::new(&context)
