...
fn cuda_j2k_encode_kernel_parameter_order_remains_stable() {
    ...
    let mut launch = fs::read_to_string(manifest.join("src/j2k_encode/launch.rs"))
        .expect("read CUDA J2K encode launch module");
    launch.push_str(
        &fs::read_to_string(manifest.join("src/j2k_encode/launch/input.rs"))
            .expect("read CUDA J2K encode input launch module"),
    );
