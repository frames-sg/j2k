...
fn assert_invalid_levels<T>(result: Result<T, CudaError>, width: u32, height: u32, requested: u8) {
    let maximum = maximum_decomposition_levels(width, height);
    match result {
...
fn safe_host_and_resident_dwt_apis_reject_degenerate_later_levels() {
    ...
    for (width, height, requested) in [
        ...
        assert_invalid_levels(
            crate::J2kCudaEngine::new(&context).j2k_forward_dwt53(&input, width, height, requested),
            width,
            height,
            requested,
        );
        assert_invalid_levels(
            crate::J2kCudaEngine::new(&context).j2k_forward_dwt97(&input, width, height, requested),
            width,
            height,
            requested,
        );
        ...
        assert_invalid_levels(
            crate::J2kCudaEngine::new(&context).j2k_forward_dwt53_resident_component(
                &components,
                0,
                width,
                height,
        ...
        assert_invalid_levels(
            crate::J2kCudaEngine::new(&context).j2k_forward_dwt97_resident_component(
                &components,
                0,
                width,
                height,
