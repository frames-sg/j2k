...
fn external_grayscale_batch_store_rejects_bounds_and_device_identity_before_launch_when_required() {
    ...
    let error = unsafe {
        crate::J2kCudaEngine::new(&context)
            .j2k_store_gray16_batch_into_external_device(&[target], &mut destination)
    }
    ...
    let error = unsafe {
        crate::J2kCudaEngine::new(&context)
            .j2k_store_gray16_batch_into_external_device(&[target], &mut foreign_destination)
    }
...
fn external_grayscale_batch_store_honors_suballocation_offsets_when_required() {
    ...
    let (ranges, _) = unsafe {
        crate::J2kCudaEngine::new(&context)
            .j2k_store_gray8_batch_into_external_device(&[target], &mut destination)
    }
