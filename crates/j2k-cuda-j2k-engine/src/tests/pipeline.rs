...
fn j2k_inverse_dwt_batch_empty_uses_no_dispatch_when_runtime_required() {
    ...
    let execution = crate::J2kCudaEngine::new(&context)
...
fn j2k_inverse_dwt_batch_odd_origin_matches_single_when_runtime_required() {
    ...
    let single = crate::J2kCudaEngine::new(&context)
...
fn j2k_inverse_dwt_batch_large_irreversible_matches_single_when_runtime_required() {
    ...
    let execution = crate::J2kCudaEngine::new(&context)
        .j2k_inverse_dwt_batch_device_with_pool_normalized(
            &[CudaJ2kIdwtTarget {
                ll: &ll,
                hl: &hl,
                lh: &lh,
...
fn j2k_inverse_dwt_batch_2592_wide_matches_single_for_both_transforms_when_runtime_required() {
    const WIDTH: usize = 2_592;
    ...
    let ll = context
        .upload(super::super::f32_slice_as_bytes(&ll_values))
        .expect("upload wide LL");
    let hl = context
        .upload(super::super::f32_slice_as_bytes(&hl_values))
        .expect("upload wide HL");
    let lh = context
        .upload(super::super::f32_slice_as_bytes(&lh_values))
        .expect("upload wide LH");
    let hh = context
        .upload(super::super::f32_slice_as_bytes(&hh_values))
        .expect("upload wide HH");
    for irreversible97 in [0, 1] {
        let band_rect = CudaJ2kRect {
            x0: 0,
            y0: 0,
            x1: band_width_u32,
        ...
        let single = crate::J2kCudaEngine::new(&context)
            .j2k_inverse_dwt_single_device_with_pool(&ll, &hl, &lh, &hh, job, &pool)
            .expect("wide single CUDA inverse DWT");
        let batch_outputs = (0..16)
            .map(|_| {
                pool.take(WIDTH * HEIGHT * std::mem::size_of::<f32>())
                    .expect("wide batched IDWT output")
            })
            .collect::<Vec<_>>();
        let targets = batch_outputs
            .iter()
            .map(|output| CudaJ2kIdwtTarget {
                ...
                output: output
                    .as_device_buffer()
                    .expect("wide batch output device buffer"),
                job,
            })
            .collect::<Vec<_>>();
        let execution = crate::J2kCudaEngine::new(&context)
            .j2k_inverse_dwt_batch_device_with_pool(&targets, &pool)
            .expect("wide batched CUDA inverse DWT");
        assert_eq!(execution.kernel_dispatches(), 2);

        let mut single_actual = vec![0.0f32; WIDTH * HEIGHT];
        single
            .buffer()
            .expect("wide single output device buffer")
            .copy_to_host(super::super::f32_slice_as_bytes_mut(&mut single_actual))
            .expect("download wide single IDWT");
        for (batch_index, batch_output) in batch_outputs.iter().enumerate() {
            let mut batch_actual = vec![0.0f32; WIDTH * HEIGHT];
            batch_output
                .copy_to_host(super::super::f32_slice_as_bytes_mut(&mut batch_actual))
                .expect("download wide batch IDWT");
            ...
            assert_eq!(
                first_mismatch, None,
                "transform mode {irreversible97}, batch {batch_index}"
            );
        }
    }
}
...
fn j2k_inverse_dwt_batch_enqueue_matches_expected_outputs_when_runtime_required() {
    ...
    let queued = unsafe {
        crate::J2kCudaEngine::new(&context).j2k_inverse_dwt_batch_device_enqueue_with_pool(
            &[CudaJ2kIdwtTarget {
                ll: &ll,
                hl: &hl,
                lh: &lh,
...
fn j2k_inverse_dwt_batch_sequence_enqueue_matches_two_stage_path_when_runtime_required() {
    ...
    // queued handle's explicit `finish` completion below.
    let queued = unsafe {
        crate::J2kCudaEngine::new(&context).j2k_inverse_dwt_batch_sequence_enqueue_with_pool(
...
fn j2k_store_rgb8_mct_matches_inverse_mct_plus_store_when_runtime_required() {
    ...
    let mct_stats = crate::J2kCudaEngine::new(&context)
        .j2k_inverse_mct_device(
            &legacy0,
            &legacy1,
            &legacy2,
            crate::j2k_decode::CudaJ2kInverseMctJob {
                len: 4,
                irreversible97: 0,
                addend0: addend,
                addend1: addend,
    ...
    let store_job = crate::j2k_decode::CudaJ2kStoreRgb8Job {
        input_width0: 2,
        input_width1: 2,
        input_width2: 2,
        source_x0: 0,
    ...
    let legacy_output = crate::J2kCudaEngine::new(&context)
        .j2k_store_rgb8_device(&legacy0, &legacy1, &legacy2, store_job)
        .expect("legacy RGB8 store");
    let fused_output = crate::J2kCudaEngine::new(&context)
        .j2k_store_rgb8_mct_device(
            &fused0,
            &fused1,
            &fused2,
            crate::j2k_decode::CudaJ2kStoreRgb8MctJob {
                store: crate::j2k_decode::CudaJ2kStoreRgb8Job {
                    addend0: addend,
                    addend1: addend,
                    addend2: addend,
                    ..store_job
...
fn j2k_store_rgb8_mct_batch_matches_separate_stores_when_runtime_required() {
    ...
    let store = crate::j2k_decode::CudaJ2kStoreRgb8Job {
        input_width0: 2,
        input_width1: 2,
        input_width2: 2,
        source_x0: 0,
    ...
    let separate_a = crate::J2kCudaEngine::new(&context)
        .j2k_store_rgb8_mct_device(
            &plane0_a,
            &plane1_a,
            &plane2_a,
    ...
    let separate_b = crate::J2kCudaEngine::new(&context)
        .j2k_store_rgb8_mct_device(
            &plane0_b,
            &plane1_b,
            &plane2_b,
    ...
    let batched = crate::J2kCudaEngine::new(&context)
        .j2k_store_rgb8_mct_batch_device(&[
            crate::j2k_decode::CudaJ2kStoreRgb8MctTarget {
                plane0: &plane0_a,
                plane1: &plane1_a,
                plane2: &plane2_a,
                job: crate::j2k_decode::CudaJ2kStoreRgb8MctJob {
                    store,
                    irreversible97: 0,
                },
            },
...
fn j2k_store_rgb8_mct_single_matches_one_item_batch_when_runtime_required() {
    ...
    let job = crate::j2k_decode::CudaJ2kStoreRgb8MctJob {
        store,
        irreversible97: 0,
    };
    let single = crate::J2kCudaEngine::new(&context)
    ...
    let batch = crate::J2kCudaEngine::new(&context)
        .j2k_store_rgb8_mct_batch_device(&[crate::j2k_decode::CudaJ2kStoreRgb8MctTarget {
            plane0: &batch0,
            plane1: &batch1,
            plane2: &batch2,
            job,
...
fn j2k_store_rgb16_mct_matches_inverse_mct_plus_store_when_runtime_required() {
    ...
    let store_job = crate::j2k_decode::CudaJ2kStoreRgb16Job {
        input_width0: 2,
        input_width1: 2,
        input_width2: 2,
        source_x0: 0,
    ...
    let fused_output = crate::J2kCudaEngine::new(&context)
        .j2k_store_rgb16_mct_device(
            ...
            &fused1,
            &fused2,
            crate::j2k_decode::CudaJ2kStoreRgb16MctJob {
                store: crate::j2k_decode::CudaJ2kStoreRgb16Job {
                    addend0: addend,
                    addend1: addend,
                    addend2: addend,
                    ..store_job
...
fn j2k_deinterleave_then_rct_can_stay_resident_when_runtime_required() {
    ...
    let context = CudaContext::system_default().expect("CUDA context");
    let mut components = crate::J2kCudaEngine::new(&context)
    ...
    let rct_execution = crate::J2kCudaEngine::new(&context)
...
#[test]
fn j2k_combined_input_compatibility_wrapper_uses_separate_dispatches_when_runtime_required() {
    if !cuda_runtime_gate() {
        return;
    }
    ...
    let mut separate = engine
        .j2k_deinterleave_to_f32_resident(&pixels, 2, 3, 8, false)
        .expect("resident CUDA deinterleave");
    engine
        .j2k_forward_rct_resident(&mut separate)
        .expect("resident CUDA forward RCT");

    let combined = engine
        .j2k_deinterleave_mct_to_f32_resident(&pixels, 2, 8, false, true)
        .expect("combined resident CUDA deinterleave and RCT");

    assert_eq!(combined.execution().kernel_dispatches(), 2);
    assert_eq!(
        combined
            .download_components()
            .expect("download combined resident components"),
        separate
            .download_components()
            .expect("download separate resident components")
    );
}
...
fn j2k_deinterleave_then_ict_can_stay_resident_when_runtime_required() {
    ...
    let ict_execution = crate::J2kCudaEngine::new(&context)
...
fn j2k_resident_deinterleave_can_feed_resident_dwt53_when_runtime_required() {
    ...
    let context = CudaContext::system_default().expect("CUDA context");
    let components = crate::J2kCudaEngine::new(&context)
    ...
    let host_component = components
        ...
        .clone();
    let expected = crate::J2kCudaEngine::new(&context)
        .j2k_forward_dwt53(&host_component, 2, 2, 1)
        .expect("host-staged CUDA DWT");

    let resident = crate::J2kCudaEngine::new(&context)
...
fn j2k_quantize_subband_matches_cpu_when_runtime_required() {
    ...
    let context = CudaContext::system_default().expect("CUDA context");
    let reversible = crate::J2kCudaEngine::new(&context)
    ...
    let irreversible = crate::J2kCudaEngine::new(&context)
        .j2k_quantize_subband(
            &samples,
            CudaJ2kQuantizeJob {
                step_exponent: 9,
