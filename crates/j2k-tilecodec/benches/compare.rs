...
fn bench_compare(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("j2k", "deflate"), |b| {
    ...
    group.bench_function(BenchmarkId::new("j2k", "zstd"), |b| {
        let mut out = vec![0_u8; source.len()];
        b.iter(|| ZstdCodec::decompress_into(&mut zstd_pool, &zstd, &mut out).expect("j2k zstd"));
    });
    ...
    group.bench_function(BenchmarkId::new("j2k", "lzw"), |b| {
        let mut out = vec![0_u8; source.len()];
        b.iter(|| LzwCodec::decompress_into(&mut lzw_pool, &lzw, &mut out).expect("j2k lzw"));
    });
    ...
    group.bench_function(BenchmarkId::new("j2k", "uncompressed"), |b| {
        let mut out = vec![0_u8; source.len()];
        b.iter(|| {
            UncompressedCodec::decompress_into(&mut no_pool, &source, &mut out).expect("j2k raw")
        });
    });
