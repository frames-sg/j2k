...
- Immediate continuation point (2026-07-13): Host/Metal/CUDA coverage ownership
  ...
  snapshot rollback are covered through `00983dd9`. The follow-up hermetic
  ...
  test tranche. The first schema-v6 exact rerun at `7238cfc5` passed Metal
  behavior and reached 79.11% critical coverage. Its remaining 48-line
  shortfall exposed the stale `/batch.rs` filename rule after the equivalent
  broad `/batch/` rule was removed; a focused regression now keeps generic
  batch coordinators out while their concrete allocation/session/buffer owners
  remain critical. The second exact run at `dc00d803` reached 79.66% and exposed
  the remaining generic keyword leak: internal `/src/compute/` routing,
  packetization, and packet-plan implementation files still entered through
  the correctness fallback. A red-green regression now assigns those broad
  algorithm internals to exact backend parity while higher-priority compute
  validation, allocation, resource, surface, buffer, and public boundaries
  remain critical. The replacement exact rerun now owns COV-001.
...
| COV-001 | P2 | in progress | METAL-001 | Exact host evidence at `954b2d74` is 88.59% overall and 87.70% critical. Exact reduced-tree Metal behavior passed at `2d3b01cc`, while schema-v5 exposed an invalid blanket denominator: 33,202 of 33,284 measurable accelerator lines were labeled critical. Schema-v6 keeps the host-wide 80% gate, enforces 80% for accelerator release-critical host boundaries, records raw accelerator implementation coverage and zero-body residuals as audit evidence, and requires exact backend parity. Red-green verification passes all 108 coverage tests, all 290 xtask unit tests, all four command-orchestration tests, all 416 active repository policies with one established ignore, strict xtask Clippy, formatting, and diff checks. The first exact schema-v6 Metal run at `7238cfc5` passed behavior and measured 79.11% critical coverage; its residual 48-line shortfall exposed one stale filename heuristic that still treated every `batch.rs` coordinator as ownership-critical despite removing the equivalent `/batch/` rule. A focused red-green correction removed only that broad suffix while retaining concrete ownership boundaries. The second exact run at `dc00d803` reached 79.66% and proved that internal `/src/compute/` routing, packetization, and packet-plan implementations still entered through generic correctness keywords. The final focused classifier regression assigns those broad algorithm internals to exact backend parity while retaining higher-priority validation, allocation, resource, surface, buffer, public-API, parser, and security boundaries. No broad execution-only test tranche is authorized. Final exact-SHA host, Metal, and CUDA artifacts remain. |
...
The exact follow-up checkpoint at
`4b80bd5c21962b1225bdc3f9708cdc5d0bd7a604` retains schema
`j2k-changed-line-coverage-v4`. The committed tranche deepens the hermetic
workspace-root child-process harness with fake `rustup`, external quality
tools, and build commands while retaining real read-only Cargo metadata. It
exercises complete package preflight, stable API and semver stale-evidence
stops, Miri, machete, typo, no-std, and fuzz-run plans. A red regression test
also proved that semver report verification incorrectly resolved
`engineering/public-api-diff-0.7.0.json` from a package working directory;
production lookup is now anchored at the workspace root. The focused 240-test
xtask binary, two orchestration integration tests, relevant repository policy,
strict all-target/all-feature Clippy, formatting, and diff checks passed before
the commit.

The clean exact-SHA run completed the full host matrix and all 411 repository
policies with one established ignore. It then failed only the genuine coverage
gates at 65,066 of 83,616 executable lines (77.8153%). Relative to `00983dd9`,
this is 302 more covered lines against three more measurable lines. The exact
80% requirement is 66,893, leaving a 1,827-line numeric gap. The report records
239 changed functions without a covered body, 424 multiline executable bodies
without a covered body, 470 zero-count deferred compiler regions, 172
compiler-noninstrumentable deferred bodies, 583 opaque macros, four absent
instrumentable files, and no mixed production/test lines.

The largest exact actionable misses are JPEG fixture builders (338 lines),
xtask code generation (176), native tile-part handling (170), Metal release
orchestration (165), release commands (152), semver (137), native packet-header
encoding (128), compare decode/comparator paths (125/124), and native decode
...
The release-status command checkpoint at
`581f48bcc16af8f05778c46c987d0020504fe934` executes the real xtask binary in
an isolated child process with fake Python and remote-origin boundaries. It
proves explicit and remote-derived repository selection, both supported token
environment names, exact SHA normalization, and help/error behavior without
network access or token-value logging. Production release-status code required
no change. The focused regression, the complete four-test orchestration suite,
all four release-status units, focused repository policy, strict xtask Clippy,
formatting, and diff checks passed before commit.

The single clean exact run at `581f48bc` completed the full host matrix and the
expanded 412-test repository policy suite with one established ignore. Child
process profiles merged into the host artifact: all five release-status
function misses closed, along with 67 uncovered lines and three opaque macros.
Numeric coverage is 65,368 of 74,738 (87.4629%). COV-001 remains open on 218
functions, 426 multiline bodies, 463 zero-count deferred regions, 172
compiler-noninstrumentable deferred bodies, 567 opaque macros, four absent
instrumentable files, and no mixed lines. JPEG benchmark support now leads
function misses at eight; the next shared tier contains several four-function
owners, including core traits, JPEG NEON, native ROI/reuse/metadata/math,
transcode metrics, facade view traits, coverage model/parsing, and CUDA
orchestration. Release commands remain the largest uncovered-line and opaque
owner at 108 and 30.

The next bounded tranches remained behavior-focused and reviewable. JPEG
benchmark support contracts gained deterministic accounting, accessor, and
detected-versus-scalar scratch tests (`0732e64e`). Native direct-ROI region
algebra gained seven boundary tests for clamping, saturation, degeneracy,
union, maximum coordinates, and conversions (`537c021d`). Release integrity
gained eight hermetic command tests covering valid traversal, aggregated
metadata failures, malformed metadata, workflow/script/document validation,
and development dependency edges (`08239578`). The compare-command tranche
then reproduced a real cleanup sequencing defect: a failed staged-input cleanup
returned before the staged output was removed. `1a6068ed` introduces one
transactional cleanup owner that always attempts both paths, preserves the
primary decode failure on best-effort cleanup, and combines dual cleanup
diagnostics. Its program override is thread-local, nestable, RAII-restored, and
non-`Send`, so parallel tests cannot consume another test's fake executable.

The first combined exact attempt correctly stopped on two integration-policy
findings before evidence was accepted. The release-integrity child recursion
guard used a public-looking `J2K_*` name even though it exists only inside test
orchestration; it now uses a private `XTASK_TEST_*` name (`c09a1c65`) rather
than polluting the supported environment-variable contract. Strict xtask
Clippy then detected that the comparator structural policy had crossed its
100-line function limit; the module and comparator assertions were split at
their actual ownership boundaries without a lint suppression or raised
threshold (`1c459f8b`). Focused release, environment-policy, comparator-policy,
format, and strict xtask Clippy checks passed before the exact rerun.

The exact host run at
`1c459f8b9e46c6586984db2aea1de688d9736ca6` completed the full host correctness
and parity matrix and all 413 repository policies with one established ignore.
Numeric coverage rose by 204 covered lines against seven additional measurable
lines, to 65,572 of 74,745 (87.7276%). The combined tranches closed 14 function
misses, four multiline-body misses, five zero-count deferred regions, 29 opaque
macros, and 197 uncovered lines. COV-001 remains open on 204 functions, 422
multiline bodies, 458 zero-count deferred regions, 172 explicitly recorded
compiler-noninstrumentable deferred bodies, 538 opaque macros, four absent
instrumentable files, and no mixed production/test lines. The report records
9,173 uncovered lines and 31,875 residual unmeasured lines. The next function
tier is four each in CUDA orchestration, coverage model/parsing, core traits,
JPEG NEON, native decode reuse/tile metadata/math, transcode metrics, and facade
view traits. The largest genuine uncovered-line owners are native tile-part
handling (103), JPEG NEON (93), semver orchestration (73), CUDA orchestration
(69), and native Tier-1/subband plus JPEG fast-420 paths (68 each). Select the
next isolated owners from this exact evidence; do not weaken the zero-body gate
or rerun the complete matrix until those focused changes compile, pass, and are
committed.

The next exact-selected tranche closed four separate responsibility families
without production behavior changes. Transcode metrics now directly exercise
empty/populated histogram length, every typed display variant, and the
saturated cap-overflow constructor (`d5198a9b`). Native SIMD assignment
operators execute one lane-preserving contract under both the active SIMD
feature and scalar no-default fallback (`232230f9`). Coverage artifact mappings
and Git revision/preflight contracts gained six focused tests covering eight
functions (`45a758ee`). Core CPU-backed decode defaults and their facade
implementations now prove borrowed-view identity, parse/construct delegation,
scratch/stride/format forwarding, output mutation, decoded rectangles, and
warning mapping (`b88dfad0`). The facade uses a real reversible 2x2 codestream;
the core boundary uses a minimal typed fake. Wildcard test imports were removed
during main-agent review before commit.

The exact host run at
`b88dfad06441dad74963ed7f12108e032cbc87d5` completed the full host correctness
and parity matrix and all 413 repository policies with one established ignore.
Numeric coverage rose by 86 covered lines with the measurable denominator
unchanged, to 65,658 of 74,745 (87.8427%). The tranche closed 25 function
misses and five opaque macros; multiline-body and deferred-region counts did
not move. COV-001 remains open on 179 functions, 422 multiline bodies, 458
zero-count deferred regions, 172 explicitly recorded compiler-noninstrumentable
deferred bodies, 533 opaque macros, four absent instrumentable files, and no
mixed production/test lines. The report records 9,087 uncovered lines and
31,881 residual unmeasured lines. `CacheStats::new`, not
`DecoderContext::new`, is the remaining `j2k-core/src/context.rs::new@17`
entry; the exact mapping corrected that review assumption. The next four-
function owners are CUDA orchestration, JPEG NEON row handling, native decode
reuse, and native tile metadata. Native tile-part handling (103), JPEG NEON
(93), semver orchestration (73), CUDA orchestration (69), and native Tier-1,
...
The next bounded tranche added typed DCT transform conversion/display/source
coverage (`23d4e779`), exercised the complete 5/3 and 9/7 JPEG-to-HTJ2K error
mapping without changing production classification (`20f22657`), proved
fallible tile-metadata clones and transactional actual-capacity replacement
ownership (`a061fc87`), and installed a test-only thread-local, nested,
RAII-restored, non-`Send` profile diagnostic sink to execute row and error
emission without changing release behavior (`873428e5`). Focused and package
tests, workspace formatting, strict package Clippy, and the relevant profile
policies passed before the commits.

The exact host run at
`873428e59c4a5ec6bdd2a3030154d2f5797d40d2` then completed the full host
correctness/parity matrix and all 413 repository policies with one established
ignore. Numeric coverage rose by 64 covered lines with no additional measurable
lines, to 65,903 of 74,747 (88.1681%). The tranche closed 13 function misses,
two multiline-body misses, and seven opaque macros; deferred-region proof did
not move. COV-001 remains open on 149 functions, 420 multiline bodies, 450
zero-count deferred regions, 172 explicitly recorded compiler-
noninstrumentable deferred bodies, 511 opaque macros, four absent
instrumentable files, and no mixed production/test lines. The report records
8,844 uncovered lines, 31,888 residual unmeasured lines, and 8,938 compiler-
noninstrumentable source lines. The next isolated function leaders are JPEG
fast-packet error classification, JPEG routing profiles, JPEG sink writers,
native HT benchmark adapters, native precomputed 5/3 APIs, and native
coefficient-source accessors at three functions each. Release command/status
still has 78 uncovered lines and three uncovered closure bodies, while the two
comparator CLI usage functions remain uncovered. These exact findings define
the next bounded parallel tranche; do not start another full matrix until it
is integrated and committed.

The next bounded tranche executed both real comparator binaries and verified
their exact help contracts (`e725868d`), exercised RGB, YCbCr, and gray sink
rows across detected and scalar-only backend tail boundaries (`d67703aa`), and
added hermetic release-status, package-gate, publish-integrity, and provenance
boundary execution (`dfce2882`). New release test owners received explicit
structural contracts (`eb328455`). The first exact attempt then exposed that
the root xtask policy crossed its existing 300-line ceiling; release-status
policy ownership moved into a cohesive child without raising the ceiling
(`56a361c8`). A second attempt exposed growth in the central ratchet registry;
the child now self-ratchets instead, again without raising a threshold
(`2f25f4a8`). The complete 414-test repository policy suite was rerun before the
final exact attempt and passed with its one established strict-only ignore.

The exact host run at
`2f25f4a8a86e086cec8455ec1c98636de5644cf6` completed the full host
correctness/parity matrix and all 413 ordinary repository policies with one
established ignore. Numeric coverage rose by 44 covered lines with no
additional measurable lines, to 65,947 of 74,747 (88.2270%). The tranche closed
five function misses, one multiline-body miss, two zero-count deferred regions,
eight opaque macros, and 44 uncovered lines. COV-001 remains open on 144
functions, 419 multiline bodies, 448 zero-count deferred regions, 172
explicitly recorded compiler-noninstrumentable deferred bodies, 503 opaque
macros, four absent instrumentable files, and no mixed production/test lines.
The report records 8,800 uncovered lines, 31,889 residual unmeasured lines, and
8,938 compiler-noninstrumentable source lines. Comparator usage and JPEG sink
writers are closed. Release command/status now has 57 uncovered lines and only
the two structurally unreachable package-graph closures. The next exact-
...
The next bounded tranche rejected ambiguous, reserved, and explicit `.git`
repository components without adding release seams (`b8e8245d`); executed J2K
and OpenJPEG full/ROI/scaled/region-scaled error, parity, batch-order,
emulation, and length-validation paths (`882f3e2f`); and captured exact full,
region, and region-scaled JPEG profile fields through a test-only thread-local,
nested, RAII-restored, non-`Send` sink (`b2ea3bb4`). Dedicated compare-decode
and JPEG-profile policy children own the new tests and reduce their parent
policies (`83258e11`).

The exact host run at
`83258e11ea3477d759de68733e42154c084b1631` completed the full host
correctness/parity matrix and all 413 ordinary repository policies with one
established ignore. Numeric coverage rose by 92 covered lines against three
additional measurable lines, to 66,125 of 74,752 (88.4592%). The tranche closed
five function misses, three multiline-body misses, 12 zero-count deferred
regions, three opaque macros, and 89 uncovered lines. COV-001 remains open on
135 functions, 415 multiline bodies, 424 zero-count deferred regions, 172
explicitly recorded compiler-noninstrumentable deferred bodies, 485 opaque
macros, four absent instrumentable files, and no mixed production/test lines.
The report records 8,627 uncovered lines, 31,896 residual unmeasured lines, and
8,938 compiler-noninstrumentable source lines. Compare decode and JPEG routing
profile emission are closed. Release command/status retains 48 uncovered lines,
but review classifies them as constant-set, numeric parse, allocation/
serialization, compile-time path, or structurally unreachable invariant guards;
do not add production seams merely to execute them. The next exact-selected
