...
impl Global {
    /// Record that a completed external GPU producer fully initialized a
    /// buffer range without asking wgpu to clear it on first use.
    ///
    /// # Safety
    ///
    /// The caller must own the range exclusively, must have resolved every
    /// prior wgpu use, and must not permit another wgpu use until the external
    /// write has completed or an equivalent GPU dependency has been installed.
    /// Every semantically accessible byte in `range` must have been written;
    /// unobservable binding-alignment padding may also be included.
    #[doc(hidden)]
    pub unsafe fn buffer_mark_external_write_initialized(
        ...
        if range.start > buffer.size {
            return Err(BufferAccessError::OutOfBoundsStartOffsetOverrun {
                index: range.start,
                max: buffer.size,
            });
        }
        if range.end < range.start || range.end > buffer.size {
        ...
        drop(buffer.initialization_status.write().drain(range));
        Ok(())
    }
