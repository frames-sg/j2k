...
impl Buffer {
    ...
    /// Tell wgpu that a completed external GPU producer fully initialized a
    /// byte range of this buffer.
    ///
    /// This narrow hook is for audited native-runtime interop. It changes only
    /// wgpu's lazy initialization tracker; it does not submit work, establish a
    /// resource transition, or synchronize either producer.
    ///
    /// # Safety
    ///
    /// - The caller must hold exclusive allocation ownership of `bounds` for
    ///   the external write. Every semantically accessible byte must have been
    ///   initialized. Backend-required binding padding may be included only
    ///   when runtime bounds checks make it unobservable.
    /// - Every prior wgpu use must be resolved before the raw write begins.
    /// - The external write must be complete, or an equivalent GPU dependency
    ///   must be registered, before any later wgpu use can observe the range.
    /// - The raw resource must not have been destroyed or replaced.
    ///
    /// # Panics
    ///
    /// Panics when `bounds` is empty or outside this buffer.
    #[cfg(wgpu_core)]
    #[doc(hidden)]
...
/// Wgpu could not accept a guarded external-write initialization handoff.
#[doc(hidden)]
#[derive(Clone, PartialEq, Eq, Debug)]
