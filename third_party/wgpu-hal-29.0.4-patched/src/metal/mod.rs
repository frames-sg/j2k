...
impl Queue {
    ...
    /// Clone the retained native `MTLCommandQueue` and transfer its +1
    /// ownership to the caller as an opaque Objective-C pointer.
    ///
    /// The caller must adopt or release the returned retain exactly once. The
    /// narrow accessor exists for guarded exact-queue identity and
    /// same-device event ordering; it does not expose queue submission
    /// through wgpu's public surface.
    #[doc(hidden)]
    pub fn retained_raw_handle(&self) -> NonNull<c_void> {
        let raw = Retained::into_raw(self.shared.raw.clone()).cast::<c_void>();
        // SAFETY: `Retained::into_raw` always returns a non-null Objective-C
        // object pointer and transfers one strong retain to the caller.
...
impl Device {
    /// Clone the retained native `MTLDevice` and transfer its +1 ownership to
    /// the caller as an opaque Objective-C pointer.
    ///
    /// This narrow escape hatch supports runtimes that must construct another
    /// audited Metal owner around the exact device selected by wgpu. The
    /// caller must adopt or release the returned retain exactly once.
    #[doc(hidden)]
    pub fn retained_raw_handle(&self) -> NonNull<c_void> {
        let raw = Retained::into_raw(self.shared.device.clone()).cast::<c_void>();
        // SAFETY: `Retained::into_raw` always returns a non-null Objective-C
        // object pointer and transfers one strong retain to the caller.
        unsafe { NonNull::new_unchecked(raw) }
    }
...
impl Buffer {
    ...
    /// Clone the retained native `MTLBuffer` and transfer its +1 ownership to
    /// the caller as an opaque Objective-C pointer.
    ///
    /// This is intentionally ownership-bearing rather than a borrowed raw
    /// handle so an external GPU submission can keep the allocation alive.
    /// The caller must adopt or release the returned retain exactly once.
    #[doc(hidden)]
    pub fn retained_raw_handle(&self) -> NonNull<c_void> {
        let raw = Retained::into_raw(self.raw.clone()).cast::<c_void>();
        // SAFETY: `Retained::into_raw` always returns a non-null Objective-C
        // object pointer and transfers one strong retain to the caller.
        unsafe { NonNull::new_unchecked(raw) }
    }
