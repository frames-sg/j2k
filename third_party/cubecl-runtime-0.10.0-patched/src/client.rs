...
impl<R: Runtime> ComputeClient<R> {
    ...
    /// Resolve the native stream owning `handle` for a guarded external write.
    ///
    /// # Safety
    ///
    /// The returned numeric handle must stay internal to an audited backend
    /// bridge. The caller must retain this client, the managed allocation, and
    /// every submitted external resource until cross-stream dependencies have
    /// been registered in both directions. It must not synchronize, destroy,
    /// cache, or expose the stream handle.
    #[doc(hidden)]
