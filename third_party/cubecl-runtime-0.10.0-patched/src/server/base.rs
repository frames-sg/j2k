...
/// Narrow backend hook used to order an audited external write on the
/// server's native stream without synchronizing the CPU.
///
/// The numeric handle is intentionally consumed only by a runtime-specific,
/// unsafe adapter. General CubeCL users should not use this trait.
#[doc(hidden)]
